...
from sympy.simplify.fu import TR3, TR5, TR6, TR7, TR8, TR10
from sympy.solvers.deutils import ode_order
from sympy.solvers.ode.systems import linodesolve, matrix_exp, matrix_exp_jordan_form
...
class MultivariableTaylorSeries(Expr):
    ...
    def _args_shifted(self, *args):
        ...
        args_shifted = {
            arg: arg - arg_shift for arg, arg_shift in self._op_point.items()
        }
    ...
    def _diff_symbols_dict(self):
        ...
        return {
            S.Zero: Subs(expr, list(op_point.keys()), list(op_point.values())),
            **{
                args_tmp: Subs(
                    Derivative(expr, *args_tmp, evaluate=False),
                    list(op_point.keys()),
                    list(op_point.values()),
                )
    ...
    def _diff_expr_dict(self):
        ...
        return {
            S.Zero: expr.subs(op_point),
            **{
                args_tmp: expr.diff(*args_tmp).subs(op_point)
                for args_tmp, poly in diff_orders_dict.items()
            },
        }
    ...
    def _components_dict(self):
        ...
        return {
            Subs(expr, list(op_point.keys()), list(op_point.values())): expr.subs(
                op_point
            ),
            **{
                derivatives_dict[args_tmp]: expr.diff(*args_tmp).subs(op_point)
                for args_tmp, poly in diff_orders_dict.items()
            },
        }
    ...
    def _series(self):
        ...
        return expr.subs(op_point).doit() + Add(
            *[
    ...
    def _symbolic_sum(self):
        ...
        return Subs(expr, list(op_point.keys()), list(op_point.values())) + Add(
            *[
                Mul(diff_dict[args_tmp], poly, evaluate=True)
                for args_tmp, poly in diff_orders_dict.items()
            ],
            evaluate=False,
        )
    ...
    def _latex(self, *args):
        ...
        return "+".join(
            [
                latex(Mul(diff_dict[args_tmp], poly, evaluate=True))
                for args_tmp, poly in diff_orders_dict.items()
            ]
        )
    ...
    def calculation_steps(self, expr_symbol=None, form=None):
        ...
        obj_sym = self.__class__(
            expr_symbol, self._vars, n=self._order, x0=self._op_point
        )
...
class AnalyticalSolution(ImmutableMatrix):
    ...
    def __new__(cls, elements, vars=None, rhs=None, evaluate=True, **options):
        ...
        if isinstance(elements, Iterable) and all(
            [isinstance(elem, Eq) for elem in elements]
        ):
    ...
    @classmethod
    def _constructor(
        cls, elements, vars=None, rhs=None, evaluate=True, ics=None, **options
    ):
    ...
    def from_eqns_list(cls, eqns_list, vars=None, **options):
        ...
        obj = cls._constructor(
            Matrix(elems), vars=vars, rhs=Matrix(values), evaluate=True, **options
        )
    ...
    def from_eqns_matrix(cls, eqns_matrix, vars=None, **options):
        ...
        obj = cls._constructor(
            Matrix(dvars), vars=vars, rhs=Matrix(values), evaluate=True, **options
        )
    ...
    def _add_rhs_form(self, other):
        if isinstance(other, AnalyticalSolution):
            ...
            dict_elems = AnalyticalSolution.from_vars_and_rhs(
                self.vars, other
            ).as_explicit_dict()

        result = Matrix(
            [self.as_explicit_dict()[coord] + dict_elems[coord] for coord in self.lhs]
        )
        return type(self).from_vars_and_rhs(self.lhs, result)
    ...
    def _op_TYPE_rhs_form(self, other, op_func):
        ...
        elems_list = [
            op_func(self.as_explicit_dict()[coord], dict_elems[coord])
            for coord in self.lhs
        ]
    ...
    def __call__(self, t, params={}):
        ...
        from ..utilities.adaptable import TimeDataFrame, TimeSeries

        solution = (
            self.nth_order_solution()
            .rhs.subs(self.extra_params)
        ...
        if isinstance(ivar, Symbol):
            ...
            nth_order_solution_fun = lambdify(
                self.ivar,
                (solution.subs(ics_dict).subs(self.extra_params).subs(ics_symbols_dict))
                .subs(self.params_values)
                .n(),
                "numpy",
            )
    ...
    def as_eq_diff_list(self, ivar, diff_order):
        ...
        It returns zip object."""
        return [
            Eq(lhs.diff(ivar, diff_order), comp.diff(ivar, diff_order), evaluate=False)
            for lhs, comp in zip(self.lhs, self.rhs)
        ]
    ...
    def _latex(self, *args):
        ...
        # display(self.rhs)
        return (
            latex(Eq(self.lhs, self.rhs, evaluate=False)) + f"~~for~~{latex(self.vars)}"
        )
    ...
    def compute_solution(
        ...
        solution_fixed = solution + Matrix(
            [
                exp(-ivar) * exp(-(1e6 + t_span[0])) if isinstance(expr, Number) else 0
        ...
        numerized_sol = TimeDataFrame(
            data={dvar: data[0] for dvar, data in zip(dvars, numerized_data)},
            index=t_span,
        )
    ...
    def _as_na_df(
        ...
        reference_data = {
            ref_val: 1 for ref_val in self.system_parameters()[params_included:]
        }
        #         reference_data = {}
        ...
        if coordinates is None:
            ...
            Y = (
                list(system._fode_dvars)
                + list(system._fode_dvars.diff(self.ivar))
                + list(dependencies_dict.keys())
            )
        else:
        ...
        df_num = NumericalAnalysisDataFrame.from_model(
            system,
...
class ODESolution(AnalyticalSolution):
    ...
    def _ics_dynamic_symbols(self):
        symbols_list = ["v", "a"]
        ics_dynamic_symbols = [
            Symbol(f"{coor}_{self._dvars_str}0") for coor in symbols_list
        ]
        ics_dynamic_symbols = Matrix(
            [Symbol(f"{self._dvars_str}0")] + ics_dynamic_symbols
        )
    ...
    def with_ics(self, ics=None, ivar0=0, sol0=None):
        ...
        return ODESolution.from_vars_and_rhs(
            self.vars, self.rhs.subs(const_dict), ics=self._ics
        )  # temporary workaround that replaces subs
...
class ODESystem(AnalyticalSolution):
    ...
    def from_ode_system(cls, ode_system):
        ...
        new_sys = cls._constructor(
            odes, dvars, odes_rhs, ivar, ode_order=ode_order, parameters=parameters
        )
    ...
    def from_dynamic_system(cls, dyn_system, ode_order=None, parameters=None):
        ...
        return cls._constructor(
            ds_lhs, dvars, ivar=ivar, ode_order=ode_order, parameters=parameters
        )
    ...
    def _constructor(
        ...
        **options,
        ...
        obj = super()._constructor(
            elements=odes, vars=dvars, rhs=odes_rhs, evaluate=evaluate, **options
        )
    ...
    def _as_msm(self, eps=None):
        ...
        msm_eq = MultiTimeScaleSolution(
            iner_mat_dim * dvars.diff(self.ivar, 2)
            + damp_mat_dim * dvars.diff(self.ivar)
            + stiff_mat_dim * dvars
            - exct_mat_dim,
            self.dvars,
            eps=eps,
    ...
    def _fode_dvars(self):
        ...
        if sys_order == 1:
            ...
            dvars_span = [
                dvar.diff(self.ivar, order)
                for order in range(sys_order)
                for dvar in self.dvars
                if order < orders_dict[dvar]
            ]
    ...
    def odes_rhs(self):
        ...
        if diffs == self.lhs:
            ...
            return diff_coeffs_mat.inv() * (
                self.rhs - self.lhs.subs({elem: 0 for elem in diffs})
            )
    ...
    def _to_rhs_ode(self):
        ...
        if diffs == self.lhs:
            ...
            if diffs == self.lhs:
                ...
                odes_rhs = (
                    -diff_coeffs_inv * coeffs_mat * self.dvars
                    + diff_coeffs_inv * free_terms
                )
            ...
            new_sys = type(self)._constructor(
                diffs,
                dvars=self.dvars,
    ...
    def odes(self):
        return Matrix(
            [
                Add(lhs, -rhs, evaluate=False) if lhs - rhs == 0 else lhs - rhs
    ...
    def _latex(self, *args):
        ...
        if self.default_ics is None:
            ...
            latex_str = (
                f"{latex(self.as_eq())}~~for~~{latex(self.dvars)}~~with~~{latex(ics)}"
            )
        #
        if len(self._callback_dict) == 0:
            return f"{latex_str}"
        else:
            cannonical_coeffs_list = [
                Eq(thing, self._callback_dict[thing], evaluate=False)
                for thing in self._callback_dict
            ]
    ...
    def _as_fode(self):
        ...
        aux_mat = Matrix(
            *fund_mat.shape,
            [dummy_list[no] if elem != 0 else 0 for no, elem in enumerate(mat_elems)],
        )
    ...
    def approximated(
        self, n=3, x0=None, op_point=False, hint=[], label=None, method=None
    ):
        """
        ...
        if method is None or method == "matrix":
            ...
            diff_matrix = Matrix(
                ...
                hd_len,
                [
                    MultivariableTaylorSeries(elem, f_ode.dvars, n=n, x0=x0).doit()
                    for elem in diff_matrix_list
                ],
            )
            ...
            lin_eqns = (
                Matrix(
                    [
                        MultivariableTaylorSeries(ode, f_ode.dvars, n=n, x0=x0).doit()
                        for ode in ode_mat
                    ]
                )
                + diff_matrix * hd_vars
            ...
            lin_eqns = Matrix(
                [
                    MultivariableTaylorSeries(
                        ode, Matrix(list(f_ode.dvars) + list(hd_vars)), n=n, x0=x0
                    ).doit()
                    for ode in ode_mat
                ]
            )
    ...
    def linearized(self, x0=None, op_point=False, hint=[], label=None, mathod=None):
        ...
        return self.approximated(
            n=1, x0=x0, op_point=op_point, hint=hint, label=label, method=method
        )
    ...
    def numerized(
        ...
        **kwrags,
        ...
        if parameter_values is None:
            parameters_sympy_dict = Dict({})
        elif isinstance(parameter_values, dict):
            parameters_sympy_dict = Dict(parameter_values)
        elif isinstance(parameter_values, Dict):
            parameters_sympy_dict = parameter_values
        else:
            print("Podano zły typ danych - podaj słownik na parameter_values")

        if ic_list is None:
            ic_tuple = ()
        elif isinstance(ic_list, list):
            ic_tuple = tuple(ic_list)
        elif isinstance(ic_list, tuple):
            ic_tuple = ic_list
        else:
            print("Podano zły typ danych - podaj list lub tuple na ic_list")

        return self._numerized(
            parameters=parameters_sympy_dict,
            ic_tuple=ic_tuple,
            backend=backend,
            expand=expand,
        )
    ...
    @lru_cache
    def _numerized(
        self, parameters=Dict(), ic_tuple=(), backend="numpy", expand=None, **kwrags
    ):
        """
    ...
    def spot_order(self):
        ...
        orders = {
            coord: max(odes.applyfunc(lambda elem: ode_order(elem, coord)))
            for coord in dvars
        }
    ...
    def _hom_equation(self):
        ...
        hom_ode_sides = ODESystem(
            self.lhs - ft_sides.lhs,
            dvars=self.dvars,
            odes_rhs=self.rhs - ft_sides.rhs,
            ode_order=self._ode_order,
            ivar=self._ivar,
        )
    ...
    def _eigenvals_simplified(self):
        ...
        delta_omg_simp = (
            omg_mean**4 - simplify(base_matrix)[1] * simplify(base_matrix)[2]
        )
        ...
        eigenval1 = Mul(
            sqrt(1 - sqrt(1 - (omg12_omg22) / omg_mean**2)),
            Mul(omg_mean_nom, 1 / omg_mean_denom, evaluate=False),
            evaluate=False,
        )
        eigenval2 = Mul(
            sqrt(1 + sqrt(1 - (omg12_omg22) / omg_mean**2)),
    ...
    def _swept_analysis(
        ...
        if subs_method is True:
            ...
            return type(self)(
                odes=self.odes.subs({freq_symbol: frequency, amp_symbol: amplitude}),
                dvars=self.dvars,
                ode_order=self._ode_order,
                ivar=self._ivar,
    ...
    def swept_analysis(
        ...
        return self._swept_analysis(
            subs_method=subs_method,
            dvar=dvar,
            freq_symbol=freq_symbol,
            amp_symbol=amp_symbol,
            amplitude=amplitude,
            ramp=ramp,
        )
    ...
    def to_canonical_form(self):
        if self._ode_order == 2 and len(self) == 1:
            ...
            new_odesys = ODESystem(
                odes=canonical_ode, dvars=Matrix([coord]), ode_order=2
            )
    ...
    def _eqn_coeffs(self):
        ...
        coeffs_dict = {
            dvar.diff(ivar, 2): system.lhs[0].coeff(
                dvar.diff(ivar, 2)
            ),  # 2nd derivative
...
class FirstOrderODESystem(ODESystem):
    ...
    def from_ode_system(cls, ode_system):
        ...
        if sys.ode_order == 1:
            new_sys = cls._constructor(
                odes, dvars, odes_rhs, ivar=ivar, ode_order=1, parameters=parameters
            )
        else:
...
class FirstOrderLinearODESystem(FirstOrderODESystem):
    ...
    def from_odes(cls, odes_system, dvars, ivar=None, ode_order=1, parameters=None):
        ...
        diffs_coeffs_mat = odes_system.jacobian(
            diffs
        )  # it should be reimplemented with .jacobian in Analytical Solution class

        if diffs_coeffs_mat == Matrix.zeros(len(diffs)):

            odes = odes_system
        else:
            odes = diffs_coeffs_mat.inv() * (
                -odes_system.subs({d_dvar: 0 for d_dvar in diffs})
            )

        f_ord_dvars = Matrix(
            sum([list(dvars.diff(ivar, no)) for no in range(ode_order)], [])
        )
        ...
        return cls._constructor(
            odes=f_ord_dvars.diff(ivar),
            dvars=f_ord_dvars,
            odes_rhs=Matrix([f_ord_dvars[len(dvars) :], odes]),
            ivar=ivar,
            ode_order=ode_order,
            parameters=parameters,
        )
    ...
    def _const_mapper(self, const_set, parameters=None):
        ...
        const_base_dict = {
            dummy_symbol: Symbol(f"C{no+1}")
            for no, dummy_symbol in enumerate(const_set)
        }
        ...
        if parameters is None:
            ...
            const_fun_dict = {
                dummy_symbol: Function(str(c_symbol))
                for dummy_symbol, c_symbol in const_base_dict.items()
            }
            const_dict = {
                dummy_symbol: c_fun(*parameters)
                for dummy_symbol, c_fun in const_fun_dict.items()
            }
    ...
    def _general_solution(self):
        ...
        A = (
            self._auxiliary_fundamental_matrix
        )  # .applyfunc(lambda elem: elem.subs(self._simp_deps,simultaneous=True))
        ...
        mapper = lambda elem: elem.subs(self._simp_deps, simultaneous=True).subs(
            self._callback_deps, simultaneous=True
        )

        CodeFlowLogger(sol, "sol list", self)

        ode_sol = (
            ODESolution.from_vars_and_rhs(self.dvars, sol)
            .applyfunc(mapper)
            .subs(const_dict)
        )
    ...
    def _steady_solution(self):
        ...
        return ODESolution.from_vars_and_rhs(self.dvars, sol).subs(
            {dum_sym: 0 for dum_sym in dummies_set}
        )
    ...
    def general_solution(self):
        ...
        rhs_ode = (
            self._to_rhs_ode()
        )  # it brings some errors - some _const_list was mapped
...
class FirstOrderLinearODESystemWithHarmonics(FirstOrderLinearODESystem):
    ...
    def _gen_sol_method_selector(self):
        ...
        if shape == (4, 4):
            ...
            top_left_minor, top_right_minor, bottom_right_minor, bottom_left_minor = (
                split_matrix(shape)
            )

            if (
                top_left_minor == Matrix([[0, 0], [0, 0]])
                and top_right_minor == Matrix([[-1, 0], [0, -1]])
                and bottom_right_minor == Matrix([0, 0], [0, 0])
            ):
    ...
    def _mdof_undamped_osc_gen_sol(self):
        ...
        solution = [
            (
                C_list[2 * i] * modes[:, i] * sin(sqrt((eigs[i, i])).doit() * t_sol)
                + C_list[2 * i + 1] * modes[:, i] * cos(sqrt(eigs[i, i]).doit() * t_sol)
            )
            * exp(0 * t_sol)
    ...
    def _sdof_osc_gen_sol(self):
        ...
        solution_position = [
            exp(-h * self.ivar)
            * (
                C_list[i] * sin(omg_h * self.ivar)
                + C_list[i + 1] * cos(omg_h * self.ivar)
            )
            for i in range(1)
        ]
        solution_velocity = [
            ...
            * (
            ...
            + exp(-h * self.ivar)
            * (
                C_list[i] * omg_h * cos(omg_h * self.ivar)
                - C_list[i + 1] * omg_h * sin(omg_h * self.ivar)
            )
            for i in range(1)
        ]
    ...
    def _sin_comp(self, omega, amp):
        ...
        if damping is not False:
            ...
            modes_trig = self._trig_modes
            eigs = [
                self._trig_eigenvalues[no, no] for no in range(len(self.dvars))[::2]
            ]
            ...
            base_sin_list = [
                (eigen**2 - omega**2)
                / ((eigen**2 - omega**2) ** 2 + (damping * omega * eigen**2) ** 2)
                for eigen in eigs
            ]
            base_cos_list = [
                -(damping * omega * eigen**2)
                / ((eigen**2 - omega**2) ** 2 + (damping * omega * eigen**2) ** 2)
                for eigen in eigs
            ]
    ...
    def _exp_sin_comp(self, omega, amp, a):
        ...
        if damping is not False:
            ...
            exp_cos_comp = Matrix(exp_cos_comp_base).row_insert(
                sl, exp_sin_comp_base * omega
            )
            exp_sin_comp = Matrix(exp_sin_comp_base).row_insert(
                sl, -exp_cos_comp_base * omega
            )
        ...
        return exp_cos_comp * exp(a * self.ivar) * cos(
            omega * self.ivar
        ) + exp_sin_comp * exp(a * self.ivar) * sin(omega * self.ivar)
    ...
    def _get_excitation_comps(self):
        ...
        """
        base_terms = self._free_terms.expand().applyfunc(
        ...
        rest = (
            base_terms
            - sum(
                [coeff * comp for comp, coeff in components],
                Matrix([0] * len(self.dvars)),
            )
        ).doit()
        ...
        for elem, coeff in self._get_excitation_comps:
            if type(elem) == cos:
                ...
                if coeff[1, 0].has(sin) == True:
                    ...
                    sol += self._exp_cos_comp(omg, 0 * b, a) + self._exp_sin_comp(
                        omg, coeff, a
                    )
                    ...
                    for arg in coeff[1, 0].args:
                        if type(arg) == cos:
                            omg = arg.args[0].diff(self.ivar).doit()
                    sol += self._exp_cos_comp(omg, coeff, a) + self._exp_sin_comp(
                        omg, 0 * b, a
                    )

            elif type(elem) == Heaviside and (
                type(elem.args[0]) in (sin, cos)
                or type(elem.args[0] * (-1)) in (sin, cos)
            ):
                ...
                # ivar0=Symbol('t0')
                sol_H = FirstOrderLinearODESystemWithHarmonics.from_ode_system(
                    odes
                ).solution.with_ics(ics_list, ivar_H_start)
                # display(sol_H)
                sol_H_out = FirstOrderLinearODESystemWithHarmonics.from_ode_system(
                    odes_out
                ).solution.with_ics(ics_list, ivar_H_end)

                from sympy import Sum, oo

                n_num = Symbol("N")

                sol_series_start = sol_H.rhs.applyfunc(
                    lambda row: Sum(
                        row * Heaviside(self.ivar - ivar_H_start),
                        (
                            n,
                            0,
                            n_num,
                ...
                sol_series_end = sol_H_out.rhs.applyfunc(
                    lambda row: Sum(
                        row * Heaviside(self.ivar - ivar_H_end), (n, 0, n_num)
                    )
                )
                ...
                sol_H = FirstOrderLinearODESystemWithHarmonics.from_ode_system(
                    odes
                ).solution.with_ics(ics_list, ivar0)
...
class FirstOrderODE:
    ...
    def __init__(
        ...
        elif all(isinstance(elem, tuple) and len(elem) == 2 for elem in params):
    ...
    def stiffness_matrix(self):
        ...
        return odes.jacobian(reg_vars).subs({coord: 0 for coord in reg_vars}).doit()
    ...
    def inertia_matrix(self):
        ...
        result = odes.jacobian(dvars_ddot).subs({coord: 0 for coord in reg_vars}).doit()
    ...
    def damping_matrix(self):
        ...
        result = odes.jacobian(dvars_dot).subs({coord: 0 for coord in reg_vars}).doit()
    ...
    def diagonalize(self):
        ...
        main_matrix = ode_sys.jacobian(self.dvars).subs(
            {dvar: 0 for dvar in self.dvars}
        )

        inertia_matrix = ode_sys.jacobian(self.dvars).subs(
            {dvar.diff(self.ivar): 0 for dvar in self.dvars}
        )  # .diff(self.ivar)
        ...
        linear_odes = inertia_matrix.inv() * (
            ...
            + ode_sys.subs({dvar: 0 for dvar in self.dvars}).doit()
        )
        ...
        #         display( list(regular_odes_list.values()) )
        regular_main_matrix = Matrix(list(regular_odes_list.values())).jacobian(
            list(regular_vars)
        )
    ...
    def steady_solution(self, initial_conditions=None):
        ...
        ext_forces = (
            self.external_forces().expand().applyfunc(lambda row: (TR8(row).expand()))
        )
        ...
        for comp in components:
            ...
            steady_sol += (sym.re(fund_mat.inv()).doit() * amp_vector) * comp + (
                sym.im(fund_mat.inv()).doit() * amp_vector
            ) * TR3(comp.subs(omg * self.ivar, omg * self.ivar - pi / 2))
        ...
        const_mat = Matrix(
            [
                sum(
                    (
                        expr
                        for expr in comp.expand().args
                        if not expr.has(self.ivar)
                        if isinstance(comp, Add)
                    ),
                    0,
                )
        ...
        if ext_forces.doit().expand() != sym.Matrix([0 for gen_coord in self.dvars]):
            #             print('o tu')
            #             display((self.governing_equations - self.external_forces() +
            #                      ext_forces).expand().doit())
            eqns_res = (
                (self.governing_equations - self.external_forces() + ext_forces)
                .expand()
                .doit()
            )
    ...
    def solution(self, initial_conditions=None):
        return {
            key: self.general_solution(initial_conditions=initial_conditions)[key]
...
class LinearODESolution:
    ...
    def stiffness_matrix(self):
        ...
        """
        return (
            self.governing_equations.jacobian(self.dvars)
            .subs({coord: 0 for coord in self.dvars})
            .doit()
        )
    ...
    def inertia_matrix(self):
        ...
        result = (
            self.governing_equations.jacobian(dvars_ddot)
            .subs({coord: 0 for coord in self.dvars})
            .doit()
        )
    ...
    def damping_matrix(self):
        ...
        return (
            self.governing_equations.jacobian(dvars_dot)
            .subs({coord: 0 for coord in self.dvars})
            .doit()
        )

    def external_forces(self):
        return self.odes_system.subs({gen_coord: 0 for gen_coord in self.dvars}).doit()
    ...
    def natural_frequencies(self):
        ...
        natural_freqs = list(
            {(sqrt(abs(eigen**2))) for eigen in self.eigenvalues() if not eigen == 0}
        )
    ...
    def steady_solution(self, initial_conditions=None):
        if (
            tuple(self.dvars),
            ImmutableMatrix(self.governing_equations),
        ) in self.__class__._cache:

            # print('cache for Linear ODE')
            steady_sol = self.__class__._cache[
                (tuple(self.dvars), ImmutableMatrix(self.governing_equations))
            ]
            ...
            ext_forces = (
                ...
                .expand()
                .applyfunc(lambda row: (TR8(row).expand()))
            )
            ...
            if ext_forces.doit().expand() != sym.Matrix(
                ...
                if len(self.dvars) == 1:
                    steady_sol += Matrix(
                        [
                            sym.dsolve(
                                eqns_res[0].expand().doit(), self.dvars[0]
                            ).rhs.subs({Symbol("C1"): 0, Symbol("C2"): 0})
                        ]
                    )
                else:
                    steady_sol += sym.dsolve(eqns_res, self.dvars)
    ...
    def solution(self, initial_conditions=None):
        return self.general_solution(
            initial_conditions=initial_conditions
        ) + self.steady_solution(initial_conditions=initial_conditions)
...
class SeparableODE(ODESystem):
    ...
    def _function_separation(self, form="expr"):
        ...
        sep_vars_dict = separatevars(
            ode_rhs.subs(dvars, dvar_sym), (ivar, dvar_sym), dict=True
        )
    ...
    def _function_integration(self, form="expr"):
        ...
        symbol_1 = Symbol(
            latex(d_var / y_sym) + "=" + latex(g_fun_ivar) + latex(d_ivar)
        )
    ...
    def _rename_variables(self):
        ...
        return {dvars: dvars_g, dvars.diff(ivar): Function(f"{dvars_g}'")(ivar)}

    def _ode_solution(self):
        dvars = self.dvars
        return ODESolution.from_vars_and_rhs(
            dvars, dsolve(self.odes[0], self.dvars[0]).rhs
        )
...
class LinearWithConstCoeffODE(ODESystem):
    ...
    def _eqn_coeffs(self):
        ...
        coeffs_dict = {
            dvar.diff(ivar): system.lhs[0].coeff(
                dvar.diff(ivar)
            ),  # coefficient of highest derivative
...
class BernoulliODE(ODESystem):
    ...
    def _as_reduced_ode(self):
        ...
        ode_rhs = (
            (
                (
                    self.subs(reduction_dict).lhs * red_expr
                    - self.subs(reduction_dict).rhs * red_expr
                )
            )
        ).expand()

        sep_ode = LinearWithConstCoeffODE(
            ode_rhs, dvars=red_dvar, ivar=self.ivar, ode_order=1
        )
