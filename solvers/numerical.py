...
from sympy.simplify.fu import TR3, TR7, TR8, TR10
...
class OdeComputationalCase:
    ...
    def __fortran_odes_rhs(self):
        ...
        subs_dict = {
            var: Symbol("temp_sym_" + str(i)) for i, var in enumerate(self.dvars)
        }
        ...
        return autowrap(
            (
                (self.odes_system).subs(
                    {self.ivar: ivar_temp, **subs_dict}, simultaneous=True
                )
            ),
            args=args_list,
        )
    ...
    def compute_solution(
        ...
        if len(t_span) > 1:
            ...
            solution_tdf = TimeDataFrame(
                data={key: solution.y[no, :] for no, key in enumerate(self.dvars)},
                index=t_span,
            )
            ...
            solution_tdf = TimeDataFrame(
                data={key: ic_list[no] for no, key in enumerate(self.dvars)},
                index=t_span,
            )
            for vel in velocities:
