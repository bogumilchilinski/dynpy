...
class ReportSimulationsAnalysis:
    ...
    def __init__(self,
        ...
        self.key_dict = {
            'given_data':
            'listed system parameters',  #to override in the particular step
    ...
    def add_described_result(self,
        ...
        step_key_max_dict = {
            str(name) + '_max': numerical_result[name].round(1).max()
            for name in column_names
        }
        step_key_min_dict = {
            str(name) + '_min': numerical_result[name].round(1).min()
            for name in column_names
        }
    ...
    def doit(self):
        ...
        generalized_coords = (next(
            (simulation_results_frame['simulations']).items())[1].columns)

        description = next(
            (simulation_results_frame['description']).items())[1]
        ...
        strings_dict = {
            ...
            'max_val': 10,
            'min_val': 10,
            'q_name': vlatex(self.analysis_key),
            't_val': 100
        }

        strings_dict = {
            **strings_dict,
            **{str(coord): vlatex(coord)
               for coord in generalized_coords}
        }

        doc = self.doc
        simulation_results_frame = self.results_frame
        doc.append(Chapter(NoEscape(self.chapter_name.format(**strings_dict))))
        ...
        with doc.create(Section('Wyniki symulacji')):
            ...
            doc.append(
                NoEscape(
                    random_sec_intro.format(**{**self.key_dict}) + '\n' * 2))

            symbols_df = pd.DataFrame(
                data=[{
                    'Zmienna':
                    '$ {eq}  $'.format(eq=vlatex(key)),
                    'Liczba przypadków':
                    len(simulation_results_frame[key].unique()),
                    'Wartość minimalna':
                    simulation_results_frame[key].min(),
                    'Wartość maksymalna':
                    simulation_results_frame[key].max(),
                    'Jednostka':
                    '$ {unit}  $'.format(unit=(self.units_dict[key]).dumps()),
                } for key in simulation_results_frame.columns
                      if isinstance(key, Symbol)]).round(2)

            doc.append(NoEscape(symbols_df.to_latex(index=False,
                                                    escape=False)))
            ...
            for no, row in enumerate(simulation_results_frame.iterrows()):
                ...
                ref_name = self.analysis_name + '_' + str(
                    self.analysis_key) + '_' + str(no)
                mrk = Marker(ref_name, prefix='fig')

                num_data = row[1][self.sims_name]
                model = row[1]['model']

                coordinates_names_dict = {
                    str(coord): vlatex(coord)
                    for coord in model.Y
                }

                step_vals_dict = {
                    key: row[1].to_dict()[key]
                    for key in model.system_parameters()
                }
                given_data_str = '$' + '$, $'.join([
                    vlatex(Eq(lhs, np.round(rhs, 1), evaluate=False)) +
                    self.units_dict[lhs].dumps()
                    for lhs, rhs in step_vals_dict.items()
                ]) + '$.'

                self.step_key_dict = {
                    **self.step_key_dict, 'given_data': given_data_str,
                    **step_vals_dict,
                    **coordinates_names_dict
                }

                self.add_described_result(numerical_result=num_data,
                                          model=model,
                                          marker=mrk,
                                          format_dict={},
                                          caption=None)
        ...
        with doc.create(
            ...
            step_key_max_dict = {
                str(name) + '_max': summary_frame[name].round(1).abs().max()
                for name in column_names
            }
            step_key_min_dict = {
                str(name) + '_min': summary_frame[name].round(1).abs().min()
                for name in column_names
            }
            step_key_idmax_dict = {
                str(name) + '_idmax':
                summary_frame[name].round(1).abs().idxmax()
                for name in column_names
            }
            step_key_idmin_dict = {
                str(name) + '_idmin':
                summary_frame[name].round(1).abs().idxmin()
                for name in column_names
            }
            ...
            step_key_max_dict = {
                str(name) + '_max': summary_frame[name].round(1).max()
                for name in column_names
            }
            step_key_min_dict = {
                str(name) + '_min': summary_frame[name].round(1).min()
                for name in column_names
            }
            step_key_idmax_dict = {
                str(name) + '_idmax': summary_frame[name].round(1).idxmax()
                for name in column_names
            }
            step_key_idmin_dict = {
                str(name) + '_idmin': summary_frame[name].round(1).idxmin()
                for name in column_names
            }
            ...
            summary_description = next(
                (simulation_results_frame['description']).items())[1]
            #print({**self.key_dict,**self.step_key_dict})
            doc.append(
                NoEscape(
                    random_conclusions.format(**{
                        **self.key_dict,
                        **self.step_key_dict
                    })))
        return doc
