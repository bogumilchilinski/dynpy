...
class ContinuousSystem:
        ...
        return sum(
            [
                ...
                * L.diff(q.diff(r_var, order + 1)).diff(r_var, order + 1)
                for r_var in self.r
                for order in range(self.diff_ord)
            ]
        ) - L.diff(q)
    ...
    def separated_vars_eqn(
        ...
        return Eq(
            (eqn_with_subs.coeff(spatial_comp) / (time_comp)),
    ...
    def spatial_general_solution(
        self, sep_expr=None, spatial_comp=Function("X")(Symbol("x"))
    ):
    ...
    def fundamental_system(
        ...
        fun_eqns = Matrix(
            [
                spatial_sol.rhs.subs(spatial_comp.args[0], flatten([key.args[-1]])[0])
                - val
        ...
        for key, val in bc_dict.items():
            ...
            matrix_comps_list += [
                (
                    key.subs(
                        {
                            spatial_sol.lhs: spatial_sol.rhs,
                            spatial_sol.lhs.subs(
                                spatial_comp.args[0], free_sym
    ...
    def eigenvalues(
        ...
        seq = SeqFormula(first_root + (index - 1) * spatial_span, (index, 1, oo))
    ...
    def eigenmodes(
        ...
        eig_value = (
            self.eigenvalues(bc_dict, sep_expr, arg, spatial_comp, index)
            .formula.expand()
            .simplify()
        )

        mode_eqn = self.fundamental_matrix(bc_dict, sep_expr, spatial_comp) * Matrix(
            C_list
        )
        # display( eig_value)
        # display( eig_value)
        ...
        mode_subs = solve(mode_eqn.applyfunc(lambda x: x.subs(eig_aid)), C_list)
        #         mode_subs = solve(mode_eqn[1:], C_list)

        #         print('solve for Cs')
        #         display(mode_subs)

        gen_sol = self.spatial_general_solution(
            sep_expr=sep_expr, spatial_comp=spatial_comp
        ).rhs
        #         display(gen_sol)
        ...
        return (
            gen_sol.subs(mode_subs)
            .subs(arg, eig_value)
            .subs({c_var: 1 for c_var in C_list})
            .subs(index, mode_no)
            .n(2)
        )
    ...
    def phase_velocity(self):
        ...
        nat_freq_eq = self.time_eqn().subs(
            {T_fun.diff(self.t, 2): -T_fun * nat_freq**2}
        )
...
class PlaneStressProblem:
    ...
    def strain_field(self, disp_func=None):
        ...
        self.strain = Matrix(
            self.dim,
            self.dim,
            [Derivative(self.u[0], self.coords[0]), 0, 0, self.u[0] / self.coords[0]],
        )
        return self.strain
    ...
    def load_strain_equation(self, dict=None):
        ...
        load_mat = self.constitutive_equation().inv() * Matrix(
            [self.strain_field()[0, 0], self.strain_field()[-1, -1]]
        )

        if dict is True:
            return {self.stress[0, 0]: load_mat[0], self.stress[-1, -1]: load_mat[-1]}
        else:
            return load_mat
    ...
    def load_equilibrium(self, volumetric_load=None):
        ...
        return (
            Derivative(self.stress[0] * self.coords[0], self.coords[0])
            - self.stress[-1]
            + self.volumetric_load * self.coords[0]
        )
    ...
    def equilibrium_eqn(self, volumetric_load=None, subs_dict=None):
        ...
        self._equilibrium_eqn = (
            self.load_equilibrium(volumetric_load=self.volumetric_load)
            .doit()
            .subs(self.load_strain_equation(dict=True))
            .subs(self._subs_dict)
            .expand()
            .simplify()
        )
    ...
    def solution(self, volumetric_load, dvar, ics=None, ivar=Symbol("r")):
        ...
        if not self._equilibrium_eqn:
            self._equilibrium_eqn = self.equilibrium_eqn(
                volumetric_load=self.volumetric_load, subs_dict=self._subs_dict
            )
        ...
        if ics is None:
            ...
            return Eq(
                ...
                -(
                    ivar
                    * (self._equilibrium_eqn.subs(dvar, 0) / ivar)
                    .doit()
                    .integrate(ivar)
                ).integrate(ivar)
                / self.D
    ...
    def integration_constants(self, volumetric_load, dvar, ics, ivar=Symbol("r")):
        ...
        self._integrantion_cs = solve(conds_eqns, symbols("C1:" + str(self.dim + 1)))

        return self._integrantion_cs

    def particular_solution(self, volumetric_load, dvar, ics, ivar=Symbol("r")):
        ...
        if not self._integrantion_cs:
            self._integrantion_cs = self.integration_constants(
                volumetric_load=self.volumetric_load, dvar=dvar, ics=ics, ivar=ivar
            )
    ...
    def loads(self, volumetric_load, dvar, ics, ivar=Symbol("r")):
        ...
        particular_sol = self.particular_solution(
            self.volumetric_load, dvar, ics, ivar=Symbol("r")
        )

        return self.load_strain_equation().subs(particular_sol.lhs, particular_sol.rhs)
