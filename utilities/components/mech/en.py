    ...
    def __init__(
        ...
        if self.system is not None:
            ...
            if len(system._given_data) == 0:
                self.append(
                    NoEscape(f"{system.readable_name} with {len(system.q)}DOF \n \n")
                )
            else:
                data_eqns = [
                    f"${latex(key)} = {latex(value)}$"
                    for key, value in system._given_data.items()
                ]

                self.append(
                    NoEscape(
                        f"""{system.readable_name} with {len(system.q)}DOF for {f', '.join(data_eqns)} \n \n"""
                    )
                )
        ...
        **kwargs,
        ...
        if self.graphic_path:
            self.append(
                Command(
                    "includegraphics",
                    options="[width=5cm,height=5cm]",
                    arguments=self.graphic_path,
                )
            )
...
class SchemeSymPyCodeComponent(ExemplaryPictureComponent):
    ...
    def header_text(self):
        return (
            "SymPy code for calling the scheme of the dynamic system is as following:"
        )
...
class PotentialEnergyComponent(ReportComponent):  # Jaś fasola
    ...
    def append_elements(self):
        ...
        display(SympyFormula(Eq(Symbol("V"), dyn_sys._potential_energy), marker=None))
...
class LagrangianComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(self.header_text.replace("marker", f"{AutoMarker(eq_lagr)}"))
        )
            ...
            display(
                (
                    SympyFormula(
                        Eq(
                            Symbol(f"X_g_-{qs}"),
                            dyn_sys_lin._ode_system.general_solution.rhs.n(3)[i],
                            evaluate=False,
                        ),
                        marker="a",
                        backend=latex,
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(
                    Symbol("F"),
                    -1 * system._left_mount.stiffness * system.z,
                    evaluate=False,
                )
            )
        )
        ...
        display(
            SympyFormula(
                Eq(
                    Symbol("F"),
                    -1
                    * system._left_mount.stiffness
                    * system._ode_system.steady_solution.rhs[0],
                    evaluate=False,
                )
            )
        )
        ...
        display(
            SympyFormula(
                Eq(
                    ...
                    -1
                    * system.c
                    * system._ode_system.steady_solution().rhs.diff(system.ivar),
                    evaluate=False,
                )
            )
        )
        ...
        display(
            SympyFormula(
                Eq(
                    ...
                    2
                    * pi
                    * system.damping_coefficient()
                    * (system.natural_frequencies()) ** -1,
                    evaluate=False,
                )
            )
        )
            ...
            display(
                (
                    SympyFormula(
                        Eq(
                            Symbol(f"X_s_-{qs}"),
                            dyn_sys_lin._ode_system.steady_solution.rhs.n(3)[i],
                            evaluate=False,
                        ),
                        marker="b",
                        backend=latex,
...
class MaxStaticForce(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(Symbol("F_s"), dyn_sys.max_static_force().doit() / 2), marker=None
            )
        )
...
class DynamicPinDiameter(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(Symbol("d"), dyn_sys.dynamic_force_pin_diameter().doit()),
                marker=None,
            )
        )
...
class StaticPinDiameter(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(Symbol("d"), dyn_sys.static_force_pin_diameter().doit()), marker=None
            )
        )
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(
                    Symbol("F"),
                    -1 * system._tensioner.stiffness * (system.z0 - system.z),
                    evaluate=False,
                )
            )
        )
        ...
        display(
            SympyFormula(
                Eq(
                    Symbol("F"),
                    -1 * system._tensioner.stiffness * system.steady_solution().rhs[0],
                    evaluate=False,
                )
            )
        )
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(
                    Symbol("F"),
                    -1 * system._tensioner_damping.c * Symbol("V"),
                    evaluate=False,
                )
            )
        )
        ...
        display(
            SympyFormula(
                Eq(
                    Symbol("F"),
                    -1
                    * system._tensioner_damping.c
                    * system.steady_solution().rhs[0].diff(system.ivar),
                    evaluate=False,
                )
            )
        )
...
class StaticKeyLength(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(Symbol("l"), dyn_sys.static_key_length().doit()), marker=None
            )
        )
...
class DynamicKeyLength(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(Symbol("l"), dyn_sys.dynamic_key_length().doit()), marker=None
            )
        )
...
class StaticAndDynamicCableForceComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(Symbol("F_s"), dyn_sys.max_static_cable_force().doit()), marker=None
            )
        )
        ...
        display(
            SympyFormula(
                Eq(Symbol("T"), dyn_sys.max_dynamic_cable_force().doit()), marker=None
            )
        )
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(
                    Eq(
                        dynamicsymbols("varphi_max").diff(dyn_sys.ivar),
                        Symbol("A_omega"),
                        evaluate=False,
                    ),
                    dyn_sys.linearized().frequency_response_function() * dyn_sys.Omega,
                    evaluate=False,
                ),
                marker=None,
            )
...
class MDoFGeneralSolutionComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        for gen_sol in (
            HarmonicOscillator(dyn_sys_lin.linearized()).general_solution().n(3)
        ):
            display(
                (
                    SympyFormula(
                        Eq(Symbol("X"), gen_sol, evaluate=False),
                        marker="a",
                        backend=latex,
                    )
                )
    ...
    def append_elements(self, phi=_phi):
        ...
        for q in dyn_sys_lin.q:
            ...
            display(
                (
                    SympyFormula(
                        Eq(Symbol("X_s"), steady_sol.as_dict()[q], evaluate=False),
                        marker="b",
                        backend=latex,
                    )
                )
...
class RightSpringForceComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(SympyFormula(Eq(Symbol("F_r"), Fr), marker=None))

        display(
            SympyFormula(Eq(Symbol("F_r"), system.right_spring_force()), marker=None)
        )
...
class CentralSpringForceComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(SympyFormula(Eq(Symbol("F_c"), Fc), marker=None))

        display(
            SympyFormula(Eq(Symbol("F_c"), system.centre_spring_force()), marker=None)
        )
...
class StaticForceComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(SympyFormula(Eq(Symbol("F"), force), marker=None))
...
class StaticCableDiameter(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(Symbol("d"), dyn_sys.static_cable_diameter().doit()), marker=None
            )
        )
...
class DynamicCableDiameter(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(Symbol("d"), dyn_sys.dynamic_cable_diameter().doit()), marker=None
            )
        )
        ...
        display(
            SympyFormula(
                Eq(
                    ...
                    (
                        (
                            list(dyn_sys()._ode_system.general_solution.atoms(sin))[
                                0
                            ].args[0]
                        )
                        / dyn_sys.ivar
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(
                    Symbol("phi"),
                    undamped.max_static_force() / dyn_sys.max_static_force(),
                ),
                marker=None,
            )
        )
...
class DampingMatrixComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(Symbol("C"), dyn_sys.damping_matrix(), evaluate=False, marker=None)
            )
        )
...
class TMDForceComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(Eq(Symbol("F_tmd"), dyn_sys.dynamic_force_TMD(), marker=None))
        )
...
class SmallParameterComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(Eq(Symbol("epsilon"), dyn_sys.small_parameter(), marker=None))
        )
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(
                    dyn_sys.qs[0],
                    solve(dyn_sys.frequency_response_function(), a)[2]
                    * sin(dyn_sys.Omega * dyn_sys.ivar),
                    marker=None,
                )
            )
        )
...
class AmplitudeAndFrequencyRelationComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(Eq(Symbol("omega") ** 2, dyn_sys.freq_amp_rel(), marker=None))
        )
...
class GivenDataComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        if given_data == {}:
            ...
            params_dict = given_data

        preview_mapper = lambda x: (
            f"${latex(x)}$" if isinstance(x, (Symbol, Eq, Function, Mul)) else str(x)
        )

        mapped_dict = {
            k: f"${latex(v)}$" if isinstance(v, (Symbol, Eq, Function, Mul)) else str(v)
            for k, v in params_dict.items()
        }

        params_dict_for_ldf = {
            "Parameter": list(params_dict.keys()),
            "Value": list(mapped_dict.values()),
        }
        ...
        display(
            ReportText(self.entry_text.format(tabelkamarker=AutoMarker(table_params)))
        )
...
class FouerierExpansionComponent(ReportComponent):
    ...
    def SeriesFormula(self):
        ...
        an = Symbol("a_n")
        bn = Symbol("b_n")
        formula = SympyFormula(
            Eq(
                kpt,
                a0 / 2
                + summation(
                    an * cos((n * pi * kpt) / T) + bn * sin((n * pi * kpt) / T),
                    (n, 1, oo),
                ),
            )
        )
    ...
    def SeriesCoeffs(self):
        ...
        coeff2 = SympyFormula(
            Eq(an, (2 / T) * integrate(kpt * cos((n * pi * t) / T), (t, -T, T)))
        )
        coeff3 = SympyFormula(
            Eq(bn, (2 / T) * integrate(kpt * sin((n * pi * t) / T), (t, -T, T)))
        )
    ...
    def append_elements(self):
        ...
        display(
            ReportText("General formula for the Fourier expansion is the following")
        )
        display(self.SeriesFormula)
        display(
            ReportText(
                "Formlas for the coefficients of the Fourier series are established in the following manner"
            )
        )
