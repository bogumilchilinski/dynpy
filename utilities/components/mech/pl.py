    ...
    def __init__(
        ...
        if self.system is not None:
            ...
            if len(system._given_data) == 0:
                self.append(
                    NoEscape(f"{system.readable_name} z {len(system.q)} St.S. \n \n")
                )
            else:
                data_eqns = [
                ...
                self.append(
                    NoEscape(
                        f"""{system.readable_name} z {len(system.q)} St.S. dla {f', '.join(data_eqns)} \n \n"""
                    )
                )
            ...
            display(
                (
                    SympyFormula(
                        Eq(
                            Symbol(f"X_g_-{qs}"),
                            dyn_sys_lin._ode_system.general_solution.n(3)[i],
                            evaluate=False,
                        ),
                        marker="a",
                        backend=latex,
...
class SteadySolutionComponent(ReportComponent):
    ...
    def append_elements(self, phi=_phi):
        ...
        dyn_sys = system
        dyn_sys_lin = dyn_sys.linearized().subs(system.get_reference_data)
...
class GivenDataComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        params_dict_for_ldf = {
            "Parametr": list(params_dict.keys()),
            "Wartość": list(mapped_dict.values()),
        }
