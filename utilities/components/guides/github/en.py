...
class CocalcDynSysListComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                " Możemy również wyświetlić podgląd takiego wahadła używając poniższej funkcji:  "
            )
        )
...
class CocalcUsageComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                "This guide concers basics of Cocalc enviroment capabilities that are crucial for explorating `DynPy` library."
            )
        )
        #         display(ReportText('Basic call of it is as follows and runs default dynamic system which is `ForcedSpringMassSystem'))
