...
class JupyterSetUpComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                "Aby elementy z których korzystamy działay należy zaimportować potrzebne biblioteki, takie jak na załączonym obrazku oraz w formie tekstu który można skopiowac"
            )
        )
...
class DynamicSystemCallComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                "Wymagane importy bibliotek/poszczególnych klas do stworzenia raportu:"
            )
        )
        ...
        display(
            ReportText(
                "Klasa umożliwiająca rozwiązanie konkretnej problematyki (w tym przypadku problematyki związanej z opisem dźwigu):"
            )
        )
...
class SimulationsComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                "Zastosowanie metody lambdify umożliwiającej konwersje funkcji do postaci anonimowej"
            )
        )
        ...
        display(
            ObjectCode("""pd.DataFrame(index=t_span, data=eq_lamdify(t_span)).plot()""")
        )

        display(
            ReportText(
                "Sposób tworzenia wykresów bezpośrednio z wcześniej wywołanej klasy:"
            )
        )
        ...
        display(
            Markdown("""table_eq=system._ode_system.steady_solution.subs(slownik)""")
        )
...
ode_sys_P = """ode_sys_P = ODESystem.from_dynamic_system(dyn_sys_P.linearized())
ode_sys_P
"""
...
class ODEReportComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                "Co więcej, istnieje też property .report, o którym informacja znajduje się rozdział niżej. Aby wygenerować ładnego pdf z użyciem         konkretnego szablonu potrzebujemy zdefiniować nasz dokuemnt jako wybrany template a następnie standardowo zaapendować sekcje w następujący sposób:"
            )
        )
...
class ODENumericalSimulationsComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                "Sprawdzenie za pomocą helpa czy tak na prawdę jest NumericalAnalysisDataFrame"
            )
        )
        display(
            ObjectCode(
...
class NumericalAnalysisSimulationComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        na_df = system.subs(param_dict).numerical_analysis(
            parameter=system.system_parameters()[0], param_span=[1, 2, 3], t_span=t_span
        )
