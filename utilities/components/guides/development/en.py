...
class DynamicSystemCompletenessCheckComponent(
    pl.DynamicSystemCompletenessCheckComponent
):
...
zadanie_6 = (
    "Wykonanie tabeli, przetworzenie jej na wykresy, opis zachodzących zależności"
)
...
class InterimScheduleComponent(ReportComponent):
    ...
    def reported_object(self):
        ...
        #             return {**default_data,'date':self._reported_object}
            ...
            return {
                **default_data,
                "date": datetime.datetime.strptime(self._reported_object, "%Y-%m-%d"),
            }
    ...
    def append_elements(self):
        ...
        first_meeting_date = self.reported_object[
            "date"
        ]  # it's useless in the case of permanent content - it's commented for future usage
        ...
        tabela_git = {
            ...
            "Data": [
                first_meeting_date + datetime.timedelta(days=days_inc.days * no)
                for no in range(10)
            ],
            "Temat konsultacji": [
                zadanie_1,
        ...
        tabelka_latex = LatexDataFrame(tabelka_pandas).reported(
            caption="Harmonogram spotkań"
        )
...
class ModellingInPythonGuidelinesComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                "Crucial information about the course on Advanced Modelling In Python"
            )
        )
        display(ObjectCode(kod_2))
        # display(ReportText('Dzięki temu możemy korzystać z przygotowanych wcześniej klas i modułów'))
...
class PythonBasicsGuidelinesComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                """A przyświeca temu idea, którą określają trzy znane cytaty:
