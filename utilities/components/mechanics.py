...
class ODESystemOperations(ReportComponent):
    ...
    def get_dummy(self):
        ...
        return ODESystem(
            odes=Matrix([m * x.diff(t, t)]),
            dvars=Matrix([x]),
            odes_rhs=Matrix([-c * x.diff(t) - b * x.diff(t, t)], ode_order=2),
        )
...
class SchemeComponent(ExemplaryPictureComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                f"""Ilustracja przedstawia schemat rzeczywistego obiektu mechanicznego, wyznaczony na podstawie uprzedniej analizy rzeczywistego obiektu.
                            """
            )
        )
        ...
        display(
            ReportText(
                f"""Analizując przedstawiony układ można stwierdzić, że jego liczba stopni swobody to {len(system.q)}.
                            """
            )
        )
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(
                    Symbol("F"),
                    -1 * system._left_mount.stiffness * system.steady_solution(),
                    evaluate=False,
                )
            )
        )
    ...
    def append_elements(self):
        ...
        display(
            SympyFormula(
                Eq(
                    Symbol("F"),
                    -1 * system.c * system.steady_solution().diff(system.ivar),
                    evaluate=False,
                )
            )
        )
...
class LagrangianComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                f"""Wyniki przedstawionych operacji wykorzystuje się wyznaczenia równań ruchu układu.
                            """
            )
        )
...
class FundamentalMatrixComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                f"""Macierz fundamentalna pozwala określić rozwiązanie ustalone. Natomiast bazując na równaniu charakterystycznym określa się częstości własne układu.
                                """
            )
        )
...
class GeneralSolutionComponent(ReportComponent):
    ...
    def append_elements(self):
        ...
        display(
            ReportText(
                f"""Rozwiązanie ogólne przedstawia wyrażenie:
                                """
            )
        )
        display(
        ...
        display(
            ReportText(
                f"""Rozwiązanie ogólne opisuje ruch analizowanego układu (przedstawia przemieszczenie w funkcji czasu) i wynika z rozważań dotyczących drgań swobodnych układu.
                                """
            )
        )
...
class SteadySolutionComponent(ReportComponent):
    ...
    def append_elements(self, phi=_phi):
        ...
        display(
            ReportText(
                f"""Rozwiązanie szczególne przedstawia wyrażenie:
                                """
            )
        )
        ...
        display(
            ReportText(
                f"""Rozwiązanie szczególne związane jest obecnością wielkości wymuszających ruch (drgania) analizowanego układu.
                                """
            )
        )
