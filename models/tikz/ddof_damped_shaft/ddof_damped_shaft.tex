...
\draw[thick,double=white,double distance=0.4cm,line cap=rect] (origo) node (start){} --++(-3cm,0) node[xshift=-0.5cm,above=+0.3cm] (Label1){$k_1, c_1$} -- ++ (-1cm,0) node (P_load){} -- ++ (-2.5cm,0) node[xshift=-1cm,above=+0.3cm] (Label2){$k_2, c_2$} -- ++ (-5.5cm,0) node (shaft_end){};
