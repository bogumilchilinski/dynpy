...
\draw[thick,double=white,double distance=0.4cm,line cap=rect] (origo) node (start){} --++(-1cm,0) node[above=+0.3cm] (Label1){} -- ++ (-1cm,0) node (P_load){} -- ++ (-4.5cm,0) node[below=+0.25cm] (Label2){$G,I_1,l_1,c_1$} -- ++ (-5.5cm,0) node (shaft_end){};

\draw[thick,double=white,double distance=0.6cm,line cap=rect] ([xshift=-0.3cm]I_1.west) --++(-2.5cm,0) node[midway,below=0.3cm] {$G,I_2,l_2,c_2$};
