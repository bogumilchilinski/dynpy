...
\tikzstyle{ground}=[fill,pattern=north east lines,draw=none,minimum width=0.75cm,minimum height=0.3cm]
   ...
   %draw axes
    ...
    \draw [spring] (M.south west)  node[] (left_spring){}  --++ (-1.5cm,-1.5cm) node[midway,right,xshift=0.3cm] {$k_{mount}$};
    
\draw [spring] (M.south east)  node[] (right_spring){}  --++ (1.5cm,-1.5cm) node[midway,left,xshift=-0.3cm] {$k_{mount}$};
...
\node (me) [fill=blue,draw,circle,outer sep=0pt,thick, minimum height=0.001cm] at (R.north east) {};
    %\fill [black] (M.center) circle (0.05);
\draw[thick, ->,] (M)[xshift=0.4cm,yshift=-0.2cm ] arc (-35:215:0.5cm) node[midway,below,yshift=-0.2cm] {$\varphi$};
...
\draw[->,transform canvas={xshift=4cm,yshift=1cm}] (origo)--++(0,2cm) node[label={right,midway:$z$}] {};
