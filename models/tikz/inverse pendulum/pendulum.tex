...
\begin{tikzpicture}[scale=2, every node/.style={scale=2}]
 ...
 \coordinate (pivot) at (1,5);
    ...
    \draw[<->,line width=1.2pt] (-0.4,0)--(0.4,0) node[midway,below]{$x_{e}(t)$};

    % draw roof
    %\fill[pattern = north east lines] ($ (origo) + (-1,0) $) rectangle ($ (origo) + (1,0.3) $);
    %\draw[thick] ($ (origo) + (-1,0) $) -- ($ (origo) + (1,0) $);

    \draw[thick] (origo) -- ++(60:3) coordinate (bob1) node[near end, right]{$m_{1}$};
    \node (l1_label) at ($(origo)+(-110:-2.1cm)$)   {$l_{1}$};
    \fill (bob1) circle (0.2);

    \pic [draw, <-, "$\varphi_{1}$", angle eccentricity=1.75] {angle = bob1--origo--mary};
\end{tikzpicture}
\end{document}
