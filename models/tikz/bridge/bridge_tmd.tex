...
\begin{tikzpicture}
    ...
    \node (p1_label) at ($(part1) + (3.2,0.7)$) [below] {$E, I,l_{beam}$};
...
\draw [spring] (part1.south)  node[] (center_spring){}  --++ (0cm,-1.1cm) node[midway,left] {$k_{TMD}$};
\node (mass) [draw,outer sep=0pt,thick,minimum width=1.5cm, minimum height=1cm, label={center:$m_{TMD}$}] at (center_spring.south)[yshift=-1.49cm] {};
