...
from .trolley import ForcedTrolleyWithThreeSprings, ThreeSprings
...
class TrolleysWithSprings(NonlinearComposedSystem):
    ...
    def components(self):
        ...
        self._spring12 = Spring(
            self.k, pos1=self.x_1, pos2=self.x_2, qs=[self.x_1, self.x_2]
        )
...
class ForcedNonLinTrolleysWithSprings(ComposedSystem):  ### 3 ODE
    ...
    def components(self):
        ...
        self._spring1 = Spring(self.k, pos1=self.x_1, qs=[self.x_1])
        self._force = Force(
            self.F * sin(self.Omega * self.ivar) + self.G, pos1=self.x_1, qs=[self.x_1]
        )
...
class ForcedDampedTrolleysWithSprings(ComposedSystem):
    ...
    def components(self):
        ...
        self._wheel11_disk = Disk(
            I=S.Half * self.m * self.R**2, pos1=self.x_l / self.R, qs=[self.x_l]
        )
        self._wheel12 = MaterialPoint(self.m, self.x_l, qs=[self.x_l])
        self._wheel12_disk = Disk(
            I=S.Half * self.m * self.R**2, pos1=self.x_l / self.R, qs=[self.x_l]
        )
        ...
        self._force = Force(
            self.F * sin(self.Omega * self.ivar) + self.F, pos1=self.x_l, qs=[self.x_l]
        )

        self._spring12 = Spring(
            2 * self.k_c, pos1=self.x_l, pos2=self.x_r, qs=[self.x_l, self.x_r]
        )
        self._damper12 = Damper(
            self.c_c, pos1=self.x_l, pos2=self.x_r, qs=[self.x_l, self.x_r]
        )
        self._wheel21 = MaterialPoint(self.m, self.x_r, qs=[self.x_r])
        self._wheel21_disk = Disk(
            I=S.Half * self.m * self.R**2, pos1=self.x_r / self.R, qs=[self.x_r]
        )
        self._wheel22 = MaterialPoint(self.m, self.x_r, qs=[self.x_r])
        self._wheel22_disk = Disk(
            I=S.Half * self.m * self.R**2, pos1=self.x_r / self.R, qs=[self.x_r]
        )
    ...
    def unit_dict(self):
        units_dict = {
            ...
            self.k_l: ureg.newton
            / ureg.meter,  # Spring constant (units depend on spring type)
            self.k_r: ureg.newton
            / ureg.meter,  # Spring constant (units depend on spring type)
            self.k_c: ureg.newton
            / ureg.meter,  # Spring constant (units depend on spring type)
            self.x_l: ureg.meter,
            self.x_r: ureg.meter,
...
class ForcedTrolleysWithNonLinearSprings(NonlinearComposedSystem):
    ...
    def components(self):
        ...
        self._trolley1 = MaterialPoint(self.m_1, self.x_1, qs=[self.x_1])
        self._spring1 = Spring(
            self.k * (self.mu * self.x_1**2), pos1=self.x_1, qs=[self.x_1]
        )
        self._force = Force(
            self.F * sin(self.Omega * self.ivar), pos1=self.x_1, qs=[self.x_1]
        )

        self._spring12 = Spring(
            self.k * (self.mu * (self.x_1 - self.x_2) ** 2),
            pos1=self.x_1,
            pos2=self.x_2,
            qs=[self.x_1, self.x_2],
        )
        self._trolley2 = MaterialPoint(self.m_2, self.x_2, qs=[self.x_2])
        self._spring2 = Spring(
            self.k * (self.mu * self.x_2**2), pos1=self.x_2, qs=[self.x_2]
        )
    ...
    def dynamic_force(self):
        ...
        return (
            (self.components["spring_1"].force().subs(amps)).subs(data).expand().doit()
        )
...
class ForcedThreeTrolleysWithSprings(ComposedSystem):  ### 7 ODE
    ...
    def components(self):
        ...
        self._spring23 = Spring(
            self.k, pos1=self.x_2, pos2=self.x_3, qs=[self.x_2, self.x_3]
        )
...
class ForcedDampedThreeTrolleysWithSprings(ComposedSystem):  ### 8 ODE
    ...
    def components(self):
        ...
        self._damper12 = Damper(
            self.c, pos1=self.x_1, pos2=self.x_2, qs=[self.x_1, self.x_2]
        )
        ...
        self._damper23 = Damper(
            self.c, pos1=self.x_2, pos2=self.x_3, qs=[self.x_2, self.x_3]
        )
...
class ForcedDampedDisconnectedTrolleysWithSprings(
...
class DDoFTwoNonLinearTrolleys(NonlinearComposedSystem):
    ...
    def components(self):
        ...
        self._spring1 = Spring(
            self.k1, pos1=(sqrt(self.x1**2 + self.d**2) - self.l_0), qs=self.qs
        )
        self._trolley2 = MaterialPoint(self.m2, self.x2, qs=self.qs)
        self._spring2 = Spring(
            self.k2, pos1=(sqrt(self.x2**2 + self.d**2) - self.l_0), qs=self.qs
        )
    ...
    def dynamic_force(self):
        ...
        return (
            (self.components["_spring12"].force().subs(data).subs(amps)).expand().doit()
        )
...
class DoubleTrolleyWithNonlinearSprings(ComposedSystem):
    ...
    def components(self):
        ...
        self._trolley_1 = ForcedTrolleyWithThreeSprings(
            self.m1,
            self.k_l,
            self.k_cl * (1 - self.mu * self.x_l**2),
            0,
            z=self.x_l,
            qs=[self.x_l],
        )
        self._three_springs = ThreeSprings(
            self.k_c,
            self.k_cc * (1 - self.mu * (self.x_r - self.x_l) ** 2),
            pos1=self.x_l,
            pos2=self.x_r,
            qs=self.qs,
        )
        ...
        self._trolley_2 = ForcedTrolleyWithThreeSprings(
            self.m2,
            self.k_r,
            self.k_cr * (1 - self.mu * self.x_r**2),
            0,
            z=self.x_r,
            qs=[self.x_r],
        )
...
class DampedTrolleysWithSprings(DoubleTrolleyDifferentWheels):
    ...
    def components(self):
        ...
        self._system = DoubleTrolleyDifferentWheels(
            R=self.R,
            m_w1=self.m,
            m_w2=self.m,
            m1=self.m1,
...
class ForcedTrolleysWithSprings(ComposedSystem):
    ...
    def components(self):
        ...
        self._trolley_1 = ForcedTrolleyWithThreeSprings(
            self.m1, self.k_l, self.k_cl, F_1, z=self.x_l, qs=[self.x_l]
        )

        self._trolley_2 = MaterialPoint(self.m2, pos1=self.x_c, qs=self.qs)
        self._springs1 = ThreeSprings(
            self.k_12, self.k_c12, pos1=self.x_l, pos2=self.x_c, qs=self.qs
        )
        self._springs2 = ThreeSprings(
            self.k_23, self.k_c23, pos1=self.x_c, pos2=self.x_r, qs=self.qs
        )

        self._trolley_3 = ForcedTrolleyWithThreeSprings(
            self.m3, self.k_r, self.k_cr, F_2, z=self.x_r, qs=[self.x_r]
        )
