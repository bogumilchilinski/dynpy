...
class AdaptableTunedFluidDamper(VariableMassTrolleyWithPendulumRayleighDamping):
    ...
    def simulation_performance(
        self, params_dict, parameter, param_span, t_span, ic_list
    ):

        na_df = self._ode_system.subs(params_dict).numerical_analysis(
            parameter=parameter, param_span=param_span
        )

        simulation_result = na_df.compute_solution(
            t_span=t_span, ic_list=ic_list, backend="numpy"
        )

        return simulation_result.droplevel(0, axis=1)

    def simulation_results_presentation(
        self, simulation_result, dynamic_variable, caption=None
    ):
        ...
        trolley_max = (
            simulation_result.loc[:, ix[:, dynamic_variable[0]]]
            .max()
            .values[0]
            .round(2)
        )
        ...
        pendulum_max = (
            simulation_result.loc[:, ix[:, dynamic_variable[1]]]
            .max()
            .values[0]
            .round(2)
        )
        ...
        mainplot_tikz_data = simulation_result.loc[:, ix[:, dynamic_variable[0]]][
            list((simulation_result.loc[:, ix[:, dynamic_variable[0]]].columns))
        ]

        mainplot_tikz = mainplot_tikz_data.to_pylatex_tikz(
            height=NoEscape(r"8cm"), width=NoEscape(r"16cm")
        ).in_figure(caption=f"Trolley displacement for various amount of {caption}")
        ...
        spectrum_tikz = spectrum_tikz_data.to_pylatex_tikz(
            height=NoEscape(r"8cm"), width=NoEscape(r"16cm")
        ).in_figure(
            caption=f"Frequency spectra of trolley displacement for various amount of {caption}"
        )

        mainplot_phi_tikz_data = simulation_result.loc[:, ix[:, dynamic_variable[1]]][
            list((simulation_result.loc[:, ix[:, dynamic_variable[1]]].columns))
        ]

        mainplot_phi_tikz = mainplot_phi_tikz_data.to_pylatex_tikz(
            height=NoEscape(r"8cm"), width=NoEscape(r"16cm")
        ).in_figure(
            caption=f"Pendulum angular displacement for various amount of {caption}"
        )

        spectrum_phi_tikz_data = (
            simulation_result.loc[:, ix[:, dynamic_variable[1]]]
            .to_frequency_domain()
            .single_sided_rms()
            .truncate(0, 1)
        ...
        spectrum_phi_tikz = spectrum_phi_tikz_data.to_pylatex_tikz(
            height=NoEscape(r"8cm"), width=NoEscape(r"16cm")
        ).in_figure(
            caption=f"Frequency spectra of pendulum angular displacement for various amount of {caption}"
        )
    ...
    def investigation_of_initial_frequency_influence(
        self, ic_list=None, t_span=None, params_list=None, params_dict=None
    ):
        ...
        if params_list is None:
            params_list = [0.494, 0.594, 0.694, 0.794, 0.894]
    ...
    def adaptable_tuned_mass_damper_validation(self):
        ...
        params_dict_2.update(
            {
                self.m_t: 8.0 * params_dict_2[self.m_p],
                self.m_tf: params_dict_2[self.m_f]
                + 0.000000001 * params_dict_2[self.t_0],
        ...
        params_dict.update(
            {
                self.m_t: 8.0 * params_dict[self.m_p],
                self.m_tf: params_dict[self.m_f]
                * (
                    (
                        S.One / 2
                        - atan(
                            params_dict[self.flow_coeff]
                            * (self.ivar - params_dict[self.t_0])
                        )
                        / pi
                    )
                ),
                self.m_pf: params_dict[self.m_f]
                * (
                    (
                        S.One / 2
                        + atan(
        ...
        simple_model_na_df = self.simulation_performance(
        ...
        advanced_model_na_df = self.simulation_performance(
            params_dict=params_dict,
            parameter=parameter,
            param_span=params_list,
            t_span=t_span,
