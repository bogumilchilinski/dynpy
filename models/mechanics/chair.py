...
class UndampedChair5DOF(ComposedSystem):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if k_rt is not None:
            self.k_rt = k_rt
        if u_rrp is not None:
            self.u_rrp = u_rrp
        if k_r is not None:
            self.k_r = k_r
        if u_rsd is not None:
            self.u_rsd = u_rsd
        if k_ft is not None:
            self.k_ft = k_ft
        if u_frp is not None:
            self.u_frp = u_frp
        if k_f is not None:
            self.k_f = k_f
        if u_fsd is not None:
            self.u_fsd = u_fsd
        if F is not None:
            self.F = F
    ...
    def components(self):
        ...
        self._body_horizontal = MaterialPoint(
            self.M, self.x + self.z_c * self.phi, qs=self.qs, frame=base_frame
        )(label="body_horizontal")
        self._body_vertical_rotational = RigidBody2D(
            self.M,
            self.I_ch,
            pos_lin=self.z,
            pos_rot=self.phi,
            qs=self.qs,
            frame=base_frame,
        )(label="body_vertical_rotational")
        self._rear_wheel_vertical_rotational = RigidBody2D(
            self.m_rear,
            self.I_w,
            pos_lin=self.z_rear,
            pos_rot=self.x / self.R,
            qs=self.qs,
            frame=base_frame,
        )(label="rear_wheel_vertical_rotational")
        self._rear_wheel_horizontal = MaterialPoint(
            self.m_rear, self.x, qs=self.qs, frame=base_frame
        )(label="rear_wheel_horizontal")
        self._front_wheel_vertical = MaterialPoint(
            self.m_fr, self.z_front, qs=self.qs, frame=base_frame
        )(label="front_wheel_vertical")
        self._front_wheel_horizontal = MaterialPoint(
            self.m_fr, self.x, qs=self.qs, frame=base_frame
        )(label="front_wheel_horizontal")
        self._front_wheel_gravity = GravitationalForce(self.m_fr, self.g, self.z_front)(
            label="front_wheel_gravity"
        )
        self._rear_wheel_gravity = GravitationalForce(self.m_rear, self.g, self.z_rear)(
            label="rear_wheel_gravity"
        )
        self._body_gravity = GravitationalForce(
            self.M, self.g, self.z + self.z_c * cos(self.phi), qs=self.qs
        )(label="body_gravity")
        self._rear_tire_spring = Spring(
            self.k_rt, self.z_rear - self.u_rrp, qs=self.qs, frame=base_frame
        )("rear_tire_spring")
        self._rear_wheel_spring = Spring(
            self.k_r, self.u_rsd, qs=self.qs, frame=base_frame
        )("rear_wheel_spring")
        self._front_tire_spring = Spring(
            self.k_ft, self.z_front - self.u_frp, qs=self.qs, frame=base_frame
        )("front_tire_spring")
        self._front_wheel_spring = Spring(
            self.k_f, self.u_fsd, qs=self.qs, frame=base_frame
        )("front_wheel_spring")
        self._hand_drive_force = Force(
            self.F * (sign(cos(self.Omega * self.ivar) - self.pm) + 1),
            pos1=self.x,
            qs=self.qs,
            frame=base_frame,
        )("hand_drive_force")
        self._hand_drive_force_phi = Force(
            self.F
            * self.Omega
            * self.R
            * (
                (sign(cos(self.Omega * self.ivar) - self.pm) + 1)
                * cos(self.Omega * self.ivar)
            ),
            pos1=self.phi,
            qs=self.qs,
            frame=base_frame,
        )("hand_drive_force_phi")

        components["_body_horizontal"] = self._body_horizontal
        components["_body_vertical_rotational"] = self._body_vertical_rotational
        components["_rear_wheel_vertical_rotational"] = (
            self._rear_wheel_vertical_rotational
        )
...
class DampedChair5DOF(UndampedChair5DOF):
    ...
    def components(self):
        ...
        self._undamped_chair_5DOF = UndampedChair5DOF(
            self.x,
            self.z,
            self.phi,
            self.z_rear,
            ...
            0,
            self.k_r,
            ((self.z + self.R * self.phi + self.l_l - self.z_rear) ** 2) - self.l_l,
            self.k_ft,
            0,
            self.k_f,
            self.ivar,
            ((self.z - self.R * self.phi + self.l_r - self.z_front) ** 2) - self.l_r,
            self.F,
            self.Omega,
            self.pm,
        )("undamped_chair_5DOF")
        self._rear_wheel_damping = Damper(
            self.I_w * self.c_mu, self.x / self.R, qs=self.qs
        )("rear_wheel_damping")
        #         self._rayleigh_damping_inertia_x = Damper(self.rayleigh_damping_inertia_x()*self.c_mu,self.x,qs=self.qs)
        self._damping_body_horizontal = Damper(
            self.M * self.c_mu, self.x + self.z_c * self.phi, qs=self.qs
        )(label="_damping_body_horizontal")
        self._damping_body_vertical = Damper(self.M * self.c_mu, self.z, qs=self.qs)(
            label="damping_body_vertical"
        )
        self._damping_body_rotational = Damper(
            self.I_ch * self.c_mu, self.phi, qs=self.qs
        )(label="damping_body_rotational")
        self._damping_rear_wheel_vertical = Damper(
            self.m_rear * self.c_mu, self.z_rear, qs=self.qs
        )(label="damping_rear_wheel_vertical")
        self._damping_rear_wheel_rotational = Damper(
            self.I_w * self.c_mu, self.x / self.R, qs=self.qs
        )(label="damping_rear_wheel_rotational")
        self._damping_rear_wheel_horizontal = Damper(
            self.m_rear * self.c_mu, self.x, qs=self.qs
        )(label="damping_rear_wheel_horizontal")
        self._damping_front_wheel_vertical = Damper(
            self.m_fr * self.c_mu, self.z_front, qs=self.qs
        )(label="damping_front_wheel_vertical")
        self._damping_front_wheel_horizontal = Damper(
            self.m_fr * self.c_mu, self.x, qs=self.qs
        )(label="damping_front_wheel_horizontal")

        self._damping_front_suspension = Damper(
            self.k_f * self.c_lam,
            ((self.z - self.R * self.phi + self.l_r - self.z_front) ** 2) - self.l_r,
            qs=self.qs,
        )(label="_damping_front_suspension")
        self._damping_rear_suspension = Damper(
            self.k_r * self.c_lam,
            ((self.z + self.R * self.phi + self.l_l - self.z_rear) ** 2) - self.l_l,
            qs=self.qs,
        )(label="damping_rear_suspension")
        self._damping_front_tire = Damper(
            self.k_ft * self.c_lam, self.z_front - self.u_frp, qs=self.qs
        )(label="damping_front_tire")
        self._damping_rear_tire = Damper(
            self.k_rt * self.c_lam, self.z_rear - self.u_rrp, qs=self.qs
        )(label="damping_rear_tire")
        ...
        components["_damping_rear_wheel_rotational"] = (
            self._damping_rear_wheel_rotational
        )
        components["_damping_rear_wheel_horizontal"] = (
            self._damping_rear_wheel_horizontal
        )
        components["_damping_front_wheel_vertical"] = self._damping_front_wheel_vertical
        components["_damping_front_wheel_horizontal"] = (
            self._damping_front_wheel_horizontal
        )
    ...
    def rayleigh_damping_inertia_x(self):
        ...
        return UndampedChair5DOF().mass_matrix * Matrix(
            self.qs
        )  # [0]#*self.c_mu#[0].coeff(self.c_mu)

    def get_param_values(self):

        default_data_dict = {self.c_mu: 0.0001, self.c_lam: 0.0001}
...
class BasicUndampedChair5DOF(ComposedSystem):
    ...
    def components(self):
        ...
        self._body_horizontal = MaterialPoint(
            self.M, self.x + self.z_c * self.phi, qs=self.qs
        )(label="body_horizontal")
        self._body_vertical_rotational = RigidBody2D(
            self.M, self.I_ch, pos_lin=self.z, pos_rot=self.phi, qs=self.qs
        )(label="body_vertical_rotational")
        self._rear_wheel_vertical_rotational = RigidBody2D(
            self.m_rear,
            self.I_w,
        ...
        self._rear_wheel_horizontal = MaterialPoint(self.m_rear, self.x, qs=self.qs)(
            label="rear_wheel_horizontal"
        )
        self._front_wheel_vertical = MaterialPoint(self.m_fr, self.z_front, qs=self.qs)(
            label="front_wheel_vertical"
        )
        self._front_wheel_horizontal = MaterialPoint(self.m_fr, self.x, qs=self.qs)(
            label="front_wheel_horizontal"
        )
        self._front_wheel_gravity = GravitationalForce(self.m_fr, self.g, self.z_front)(
            label="front_wheel_gravity"
        ...
        self._rear_tire_spring = Spring(
            self.k_rt, self.z_rear - self.u_rrp, qs=self.qs
        )("rear_tire_spring")
        self._rear_wheel_spring = Spring(self.k_r, self.u_rsd, qs=self.qs)(
            "rear_wheel_spring"
        )
        self._front_tire_spring = Spring(
            self.k_ft, self.z_front - self.u_frp, qs=self.qs
        )("front_tire_spring")
        self._front_wheel_spring = Spring(self.k_f, self.u_fsd, qs=self.qs)(
            "front_wheel_spring"
        )
        self._hand_drive_force = Force(
            self.F * (sign(cos(self.Omega * self.ivar) - self.pm) + 1),
...
class BasicDampedChair5DOF(BasicUndampedChair5DOF):
    ...
    def components(self):
        ...
        self._undamped_chair_5DOF = BasicUndampedChair5DOF(
            self.x,
            self.z,
            self.phi,
            self.z_rear,
        ...
        self._damping_front_suspension = Damper(
            self.k_f * self.c_lam, self.u_fsd, qs=self.qs
        )(label="_damping_front_suspension")
        self._damping_rear_suspension = Damper(
            self.k_r * self.c_lam, self.u_rsd, qs=self.qs
        )(label="damping_rear_suspension")
        self._damping_front_tire = Damper(
            self.k_ft * self.c_lam, self.z_front - self.u_frp, qs=self.qs
        )(label="damping_front_tire")
    ...
    def get_param_values(self):
        ...
        self.default_data_dict = {self.c_mu: 0.0001, self.c_lam: 0.0001}
...
class LinearSpringDeflectionUndampedChair5DOF(BasicUndampedChair5DOF):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if pm is not None:
            self.pm = pm
        if l_rear is not None:
            self.l_rear = l_rear
        if l_front is not None:
    ...
    @cached_property
    def components(self):
        rear_spring_deflection = (
            (self.z + self.R * self.phi + self.l_rear - self.z_rear) ** 2
        ) - self.l_rear
        front_spring_deflection = (
            (self.z - self.R * self.phi + self.l_front - self.z_front) ** 2
        ) - self.l_front
        rear_road_profile = 0
        front_road_profile = 0
        components = {}

        self._basic_undamped_chair_5DOF = BasicUndampedChair5DOF(
            self.x,
            self.z,
            self.phi,
            self.z_rear,
    ...
    def symbols_description(self):
        self.sym_desc_dict = {self.l_rear: "???", self.l_front: "???"}

        return {**self.sym_desc_dict, **super().symbols_description()}


class LinearSpringDeflectionDampedChair5DOF(
    LinearSpringDeflectionUndampedChair5DOF, BasicDampedChair5DOF
):
    ...
    def components(self):
        ...
        self._basic_damped_chair_5DOF = BasicDampedChair5DOF(
            self.x,
            self.z,
            self.phi,
            self.z_rear,
...
class NonLinearSpringDeflectionUndampedChair5DOF(
    LinearSpringDeflectionUndampedChair5DOF
):
    ...
    def components(self):
        rear_spring_deflection = (
            sqrt(
                (self.z + self.R * sin(self.phi) + self.l_rear - self.z_rear) ** 2
                + (self.R - self.R * cos(self.phi)) ** 2
            )
            - self.l_rear
        )
        front_spring_deflection = (
            sqrt(
                (self.z - self.R * sin(self.phi) + self.l_front - self.z_front) ** 2
                + (self.R - self.R * cos(self.phi)) ** 2
            )
            - self.l_front
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        if omega is not None:
            self.omega = omega
        if I_ch is not None:
            self.I_ch = I_ch  # moment of inertia of a rod
        if I_w is not None:
            self.I_w = I_w
        if F_engine is not None:
            self.F_engine = F_engine
        if l_bumps is not None:
            self.l_bumps = l_bumps
        ...
        self.spring_lw = Spring(
            self.k_rt, pos1=self.z_rear, pos2=self.s, qs=[self.z_rear]
        )
        self.damper_pw = Damper(self.c_ft, pos1=self.z_fr, pos2=self.s, qs=[self.z_fr])
        self.damper_lw = Damper(
            self.c_rt, pos1=self.z_rear, pos2=self.s, qs=[self.z_rear]
        )
        ...
        self.front_wheel_rot = MaterialPoint(
            self.I_w, pos1=self.x / self.R, qs=[self.x]
        )
        #         self.pasazer= MaterialPoint(self.m_k, pos1=self.z+self.phi*self.l_k, qs=[self.z, self.phi])
        self.spring_1 = Spring(
            self.k_r,
            pos1=self.z + self.phi * self.l_l,
            pos2=self.z_rear,
            qs=[self.z, self.phi, self.z_rear],
        )  # left spring
        self.spring_2 = Spring(
            self.k_f,
            pos1=self.z - self.phi * self.l_r,
            pos2=self.z_fr,
            qs=[self.z, self.phi, self.z_fr],
        )
        self.damper_1 = Damper(
            self.c_rs,
            pos1=self.z + self.phi * self.l_l,
            pos2=self.z_rear,
            qs=[self.z, self.phi, self.z_rear],
        )
        self.damper_2 = Damper(
            self.c_fs,
            pos1=self.z - self.phi * self.l_r,
            pos2=self.z_fr,
            qs=[self.z, self.phi, self.z_fr],
        ...
        self.gravitational_force = GravitationalForce(
            self.M, self.g, (self.z + self.z_c3) * cos(self.phi), qs=self.qs
        )
        ...
        system = (
            ...
            + self.damper_x
            + self.force_1
            + self.front_wheel_hor
            + self.front_wheel_rot
            + self.rear_wheel_hor
    ...
    def __init__(
        ...
        **kwargs
        ...
        if l_l is not None:
            self.l_l = l_l  # offset of left spring
        if l_p is not None:
            self.l_p = l_p  # offset of right spring
        if l_b is not None:
            self.l_p = l_b
        if l_rod is not None:
            self.l_rod = l_rod
        ...
        self.right_wheel = MaterialPoint(
            self.m_p, pos1=self.z_pw, qs=[self.z, self.phi]
        )
        self.left_wheel = MaterialPoint(self.m_l, pos1=self.z_lw, qs=[self.z, self.phi])
        self.spring_pw = Spring(
            self.k_pw, pos1=self.z_pw, pos2=self.s, qs=[self.z, self.phi]
        )
        self.spring_lw = Spring(
            self.k_lw, pos1=self.z_lw, pos2=self.s, qs=[self.z, self.phi]
        )
        self.damper_pw = Damper(
            self.c_pw, pos1=self.z_pw, pos2=self.s, qs=[self.z, self.phi]
        )
        self.damper_lw = Damper(
            self.c_lw, pos1=self.z_lw, pos2=self.s, qs=[self.z, self.phi]
        )
        # self.spring=Spring(k,pos1,po2,qs)

        self.body = RigidBody2D(
            self.m, self.I, pos_lin=self.z, pos_rot=self.phi, qs=[self.z, self.phi]
        )  # rod
        self.battery = MaterialPoint(
            self.m_b, pos1=self.z + self.phi * self.l_b, qs=[self.z, self.phi]
        )
        self.spring_1 = Spring(
            self.k_l, pos1=self.z_l, pos2=self.z_lw, qs=[self.z, self.phi]
        )  # left spring
        self.spring_2 = Spring(
            self.k_p, pos1=self.z_p, pos2=self.z_pw, qs=[self.z, self.phi]
        )
        self.damper_1 = Damper(
            self.c_l, pos1=self.z_p, pos2=self.z_lw, qs=[self.z, self.phi]
        )
        self.damper_2 = Damper(
            self.c_p, pos1=self.z_l, pos2=self.z_pw, qs=[self.z, self.phi]
        )
...
class DampedChairSimplifiedDDOF2(ComposedSystem):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.spring_1 = Spring(
            self.k_l_zas, pos1=self.z_l, pos2=self.s, qs=[self.z, self.phi]
        )  # left spring
        self.spring_2 = Spring(
            self.k_p_zas, pos1=self.z_p, pos2=self.s, qs=[self.z, self.phi]
        )
        self.damper_1 = Damper(
            self.c_l_zas, pos1=self.z_p, pos2=self.s, qs=[self.z, self.phi]
        )
        self.damper_2 = Damper(
            self.c_p_zas, pos1=self.z_l, pos2=self.s, qs=[self.z, self.phi]
        )
