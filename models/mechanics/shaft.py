...
class SDoFShaft(ComposedSystem):
    ...
    def max_dynamic_force(self):
        ...
        return (
            ...
            * self.spring_2.stiffness
            * 2
            / d
            + self.max_static_force()
        )
...
class SDoFShaftHarmonicExcitation(SDoFShaft):
    ...
    def get_default_data(self):
        ...
        m0, l0, G, l, theta0, Omega0 = symbols(
            "m_0 l_0 G l theta_0 Omega_0", positive=True
        )
    ...
    def max_dynamic_force(self):
        ...
        #             self.natural_frequencies()[0]) * self.stiffness_matrix()[0]
        return (
            self.subs({self.theta: theta0}).frequency_response_function()
            * self.spring_2.stiffness
            * 2
            / d
            + self.max_static_force()
...
class DoubleDiskShaft(ComposedSystem):
    ...
    def components(self):
        ...
        self.spring_1 = Spring(
            self.k_1, self.phi_1, self.phi_2, qs=self.qs
        )  # left spring

        self.disc_2 = Disk(self.I_m2, pos1=self.phi_2, qs=self.qs)
        self.spring_2 = Spring(
            self.k_2, pos1=self.phi_2, pos2=self.theta, qs=self.qs
        )  # right spring
    ...
    def dynamic_force(self):
        ...
        dyn_force = (
            (self.components["spring_2"].force().subs(amps)).subs(data).expand().doit()
        )
    ...
    def max_dynamic_disk_1_bearing_force(self):
        ...
        return (
            abs(2 * (self.I_m1 * acc_amp) / d) + self.max_static_disk_1_bearing_force()
        )  # .subs(self._given_data)
    ...
    def max_dynamic_disk_2_bearing_force(self):
        ...
        return (
            abs(2 * (self.I_m2 * acc_amp) / d) + self.max_static_disk_2_bearing_force()
        )  # .subs(self._given_data)
...
class DoubleDiskShaftHarmonicExcitation(DoubleDiskShaft):
    ...
    def components(self):
        ...
        self.spring_2 = Spring(
            self.k_2,
            pos1=self.phi_2,
            pos2=self.theta * cos(self.Omega * self.ivar),
            qs=self.qs,
        )  # right spring
        self.moment_disc1 = Force(self.T_1, pos1=self.phi_1)
        self.moment_disc2 = Force(self.T_2, pos1=self.phi_2)
...
class TripleShaft(ComposedSystem):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if phi_r is not None:
            self.phi_r = phi_r
        if input_displacement is not None:
    ...
    def max_dynamic_disk_1_bearing_force(self):
        ...
        acc_amp = (
            (self.phi_1)
            .subs(self._given_data)
            .subs({self.phi_l: frf[0], self.phi_r: frf[1]})
        )
    ...
    def unit_dict(self):
        units_dict = {
            ...
            self.phi_1.diff(self.ivar): ureg.radian
            / ureg.second,  # Angular velocity of the first disk (rad/s)
            self.phi_1.diff(self.ivar, 2): ureg.radian
            / ureg.second**2,  # Angular acceleration of the first disk (rad/s**2)
            self.phi_2.diff(self.ivar): ureg.radian
            / ureg.second,  # Angular velocity of the second disk (rad/s)
            self.phi_2.diff(self.ivar, 2): ureg.radian
            / ureg.second**2,  # Angular acceleration of the second disk (rad/s**2)
            self.phi_3.diff(self.ivar): ureg.radian
            / ureg.second,  # Angular velocity of the third disk (rad/s)
            self.phi_3.diff(self.ivar, 2): ureg.radian
            / ureg.second**2,  # Angular acceleration of the third disk (rad/s**2)
...
class SDoFDampedShaft(ComposedSystem):
    ...
    def components(self):
        ...
        self.damper_1 = Damper(
            self.c_1 * self.c_2 / (self.c_2 + self.c_1),
            pos1=self.phi,
            pos2=self.theta,
            qs=self.qs,
        )
