...
class BeamBridge(ComposedSystem):
    ...
    def components(self):
        ...
        self._mass = BeamStructure(self.m, self.z, qs=[self.z])(label="Mass of a beam")
        self._spring = BeamElasticity(self.k_beam, self.z, qs=[self.z])(
            label="Beam stiffness"
        )
        self._gravity_force = GravitationalForce(self.m, self.g, self.z)(
            label="Gravity field"
        )
        self._force = Force(-self.F * sin(self.Omega * self.ivar), pos1=self.z)(
    ...
    def dynamic_force(self):
        ...
        amps = self._fodes_system.steady_solution.as_dict()
        dyn_force = (
            (self.components["_spring"].force().subs(amps)).subs(data).expand().doit()
        )
...
class BeamBridgeDamped(BeamBridge):
    ...
    def components(self):
        ...
        self._beam_bridge = BeamBridge(
            self.m, self.k_beam, self.ivar, self.g, self.Omega, self.F
        )(label="Beam Bridge")
        self._damper = Damper(self.c, pos1=self.z, qs=[self.z])(
            label="Damping of a beam"
        )
    ...
    def dynamic_force(self):
        ...
        amps = self._fodes_system.steady_solution.as_dict()
        dyn_force = (
            (self._beam_bridge.components["_spring"].force().subs(amps))
            .subs(data)
            .expand()
            .doit()
        )
...
class BeamBridgeTMD(BeamBridge):
    ...
    def components(self):
        ...
        self._TMD = TunedMassDamper(self.m_TMD, self.k_TMD, self.z_TMD, self.z)(
            label="TMD (damper)"
        )
        self._TMD_gravity = GravitationalForce(self.m_TMD, self.g, self.z_TMD)(
            label="Gravity field for TMD"
        )
...
class BeamBridgeDampedTMD(BeamBridge):
    ...
    def components(self):
        ...
        self._beam_bridge_damped = BeamBridgeDamped(
            self.m, self.k_beam, self.ivar, self.g, self.Omega, self.F, self.c
        )(label="Beam Bridge Damped")
        self._TMD = TunedMassDamper(self.m_TMD, self.k_TMD, self.z_TMD, self.z)(
            label="TMD (damper)"
        )
        self._TMD_gravity = GravitationalForce(self.m_TMD, self.g, self.z_TMD)(
        ...
        self._damper_TMD = Damper(
            self.c_TMD, pos1=self.z - self.z_TMD, qs=[self.z, self.z_TMD]
        )(label="Damper of a TMD")
    ...
    def get_default_data(self):
        ...
        default_data_dict = {
            ...
            self.k_TMD: [
                S.One * no * (48 * E * I / self.l**3) * 1000 for no in range(10, 20)
            ],
        }
