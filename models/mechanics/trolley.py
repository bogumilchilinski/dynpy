...
class VariableMassTrolleyWithPendulum(ComposedSystem):
    ...
    t_init = Symbol('t_0', positive=True)
    ...
    def __init__(self,
        ...
        if t_init is not None: self.t_init = t_init
        if g is not None: self.g = g
        if phi is not None: self.phi = phi
        if x is not None: self.x = x
        if k is not None: self.k = k
...
class VariableMassTrolleyWithPendulumRayleighDamping(ComposedSystem):
    ...
    def components(self):
        ...
        self.m_tf_eq = self.m_f*((S.One/2-atan(self.flow_coeff*(self.ivar-self.t_init))/pi))
        self.m_pf_eq = self.m_f*((S.One/2+atan(self.flow_coeff*(self.ivar-self.t_init))/pi))
...
#         return default_data_dict
    ...
    def get_numerical_data(self):
        ...
        default_data_dict.update({self.m_t: [10.0*default_data_dict[self.m_p][0]],
                                  self.l: [0.99*default_data_dict[self.g][0]/(default_data_dict[self.Omega][0]*default_data_dict[self.Omega][0])],
                                  self.k: [2.0*default_data_dict[self.m_p][0]*default_data_dict[self.g][0]/(default_data_dict[self.g][0]/default_data_dict[self.Omega][0]**2)],
                                  self.m_tf: [default_data_dict[self.m_f][0]*((S.One/2-atan(default_data_dict[self.flow_coeff][0]*(self.ivar-default_data_dict[self.t_init][0]))/pi))],
                                  self.m_pf: [default_data_dict[self.m_f][0]*((S.One/2+atan(default_data_dict[self.flow_coeff][0]*(self.ivar-default_data_dict[self.t_init][0]))/pi))],
                                          })
