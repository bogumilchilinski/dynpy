...
class RollingDisk(ComposedSystem):
    ...
    def __init__(self, m1=None, R=None, x=None, qs=None, ivar=Symbol("t"), **kwargs):
    ...
    def components(self):
        ...
        self._disk_rot = MaterialPoint(
            self.m1 / 2 * self.R**2, self.x / self.R, qs=self.qs
        )
...
class DiskMountingBlock(ComposedSystem):
    ...
    def __init__(
        self, m=None, m1=None, R=None, x=None, qs=None, ivar=Symbol("t"), **kwargs
    ):
    ...
    def components(self):
        ...
        self._disk = RollingDisk(
            m1=self.m, R=self.R, x=self.x, qs=[self.x], ivar=self.ivar
        )
...
class DiskWithNonlinearSpring(NonlinearComposedSystem):
    ...
    def components(self):
        ...
        self._spring = Spring(
            self.kl, pos1=(sqrt(self.x**2 + self.d**2) - self.l_0), qs=self.qs
        )
...
class ForcedDiskWithNonlinearSpring(NonlinearComposedSystem):
    ...
    def components(self):
        ...
        self._disk = DiskWithNonlinearSpring(
            self.m1, self.kl, self.R, self.d, self.l_0, self.x, self.ivar
        )
        self._force = Force(
            self.F * cos(self.Omega * self.ivar), pos1=self.x, qs=self.qs
        )
...
class DampedDiskWithNonlinearSpring(
    NonlinearComposedSystem
):  # trzeba zupdateowac scheme - dodac tlumienie liniowe
    scheme_name = "nonlinear_disc.png"
    real_name = "roller_tightener.png"
    ...
    def components(self):
        ...
        self._disk = ForcedDiskWithNonlinearSpring(
            self.m1,
            self.kl,
            self.R,
            self.d,
...
class RollingHalfDisk(ComposedSystem):
    ...
    def __init__(
        self, r=None, m=None, g=None, ivar=Symbol("t"), phi=None, qs=None, **kwargs
    ):
...
class ForcedRollingHalfDisk(RollingHalfDisk):
    ...
    def components(self):
        ...
        self.rolling_half_disk = RollingHalfDisk(
            m=self.m, r=self.r, phi=self.phi, qs=self.qs
        )
        self.torque = Force(
