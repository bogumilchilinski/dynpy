...
class BlowerToothedBelt(ComposedSystem):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if m is not None:
            self.m = m
        if k_belt is not None:
            self.k_belt = k_belt
        if k_tensioner is not None:
    ...
    def components(self):
        ...
        self._upper_belt = Spring(self.k_belt, self.z, pos2=0, qs=[self.z])(
            label="Upper belt stiffness"
        )
        self._lower_belt = Spring(self.k_belt, self.z, pos2=0, qs=[self.z])(
            label="Lower belt stiffness"
        )
        self._tensioner = Spring(self.k_tensioner, self.z, pos2=self.z0, qs=[self.z])(
            label="Tensioner stiffness"
        )
        self._force = Force(
            self.F * sin(self.Omega * self.ivar), pos1=self.z, qs=[self.z]
        )(label="Force")
    ...
    def dynamic_force(self):
        ...
        force_in_tensioner = (
            self.components["_tensioner"].force().doit().expand()
        )  # .doit()
        data = self._given_data
...
class DampedBlowerToothedBelt(BlowerToothedBelt):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if lam is not None:
            self.lam = lam
        if c_belt is not None:
            self.c_belt = c_belt
        if c_tensioner is not None:
    ...
    def components(self):
        ...
        self._upper_belt_damping = Damper(
            self.c_belt, pos1=self.z, pos2=0, qs=[self.z]
        )(label="Upper belt damping")
        self._lower_belt_damping = Damper(
            self.c_belt, pos1=self.z, pos2=0, qs=[self.z]
        )(label="Lower belt damping")
        self._tensioner_damping = Damper(
            self.c_tensioner, pos1=self.z, pos2=self.z0, qs=[self.z]
        )(label="Tensioner damping")
