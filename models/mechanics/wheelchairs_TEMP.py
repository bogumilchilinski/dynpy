...
### system parameters
m_fr, m_rear, m_3, k_r, k_rt, k_f, k_ft, k_rot, amplitude, length, speed, axw = symbols(
    "m_fr, m_r, M, k_r, k_rt, k_f, k_ft k_rot, A, L, v, W_wb ", positive=True
)
m, k, g, F_1, F_2, Omega, F, R, v0, u0, l_l, l_r = symbols(
    "m,k,g,F_1,F_2,Omega, F_0, R, v_0,u_0,l_l,l_r", positive=True
)
I_ch, I_w, z_c3, l_fr, l_rear = symbols(
    "I_chair, I_wheel, z_c3, l_fr, l_r", positive=True
)
m_RC, I_RC, l_RC, k_RC, phi0 = symbols(
    "m_RC, I_RC, l_RC, k_RC, varphi_0", positive=True
)
m_w, I_wrc, r_w, k_w, k_fix, k_tire = symbols(
    "m_w, I_wRC, R_w, k_w, k_fix, k_t", positive=True
)
...
x, z_fr, z_rear, z, phi, phi_rc, theta, z_wrc = dynamicsymbols(
    "x,z_fr,z_r,z, varphi,varphi_RC, theta, z_wrc"
)
dx, dz_fr, dz_rear, dz, dphi, dphi_rc, dtheta, dz_wrc = dynamicsymbols(
    "x,z_fr,z_r,z, varphi,varphi_RC, theta, z_wrc", 1
)

var_dynamic = {
    x: "przemieszczenie poziome wózka",
...
u_rf_bump_on = u0 / 2 * (1 + 2 / pi * atan(5 * (t - t0))) - u0 / 2 * (
    1 + 2 / pi * atan(5 * (t - t0 - t_l))
)
u_rf_bump = S.Zero
...
T_body = (
    S.One / 2 * m_3 * x_m3.diff(t) ** 2 + S.One / 2 * m_3 * dz**2
)  # Ek ramy wózka w ruchu postępowym
T_rot = S.One / 2 * I_ch * dphi**2  # Ek ramy wózka w ruchu obrotowym
T_rear = (
    S.One / 2 * m_rear * dz_rear**2 + S.One / 2 * m_rear * dx**2
)  # Ek tylniego koła Ir (mr)
T_fr = (
    S.One / 2 * m_fr * dz_fr**2 + S.One / 2 * m_fr * dx**2
)  # Ek przedniego koła If (mf)
...
V_chair_g = (
    m_fr * g * z_fr + m_rear * g * z_rear + m_3 * g * (z + z_c3 * (cos(phi)))
)  # EPG wózka względem punktu cięzkości wózka
...
# spring potentials
V_rear = (
    S.One / 2 * k_rt * (z_rear - u_rr) ** 2 + S.One / 2 * k_r * (u_rear) ** 2
)  # Ep tylnich prętów ramy względem ich sprężystości
V_fr = (
    S.One / 2 * k_ft * (z_fr - u_rf) ** 2 + S.One / 2 * k_f * (u_fr) ** 2
)  # Ep przednich prętów ramy względem ich sprężystości
...
Pl.set_vel(
    N,
    (u[0] + u[1] * R * sin(Omega * t).diff(t)) * N.x
    + (u[2] * R * cos(Omega * t).diff(t)) * N.y,
)
Pr.set_vel(N, u[0] * N.x)

points_list = [
    Point("P_" + str(no)) for no, vel in enumerate(Matrix(qs_chair5dof_rc_3dof).diff(t))
]
[
    points_list[no].set_vel(N, vel * N.x)
    for no, vel in enumerate(Matrix(qs_chair5dof_rc_3dof).diff(t))
]
...
FL_chair5dof = [(Pl, 0.5 * (1 * sign(cos(Omega * t) - pm) + 1) * 2 * F * N.x)] + [
    (points_list[no], -D_chair5dof.diff(vel) * N.x)
    for no, vel in enumerate(Matrix(qs_5dof).diff(t))
]

FL_rapidchair3dof = [
    (points_list[no], -D_rapidchair3dof.diff(vel) * N.x)
    for no, vel in enumerate(Matrix(qs_rc_3dof).diff(t))
]
...
FL_chair5dof_rc3dof = [
    (Pl, 0.5 * (1 * sign(cos(Omega * t) - pm) + 1) * 2 * F * N.x)
] + [
    (points_list[no], -D_chair5dof_rc3dof.diff(vel) * N.x)
    for no, vel in enumerate(Matrix(qs_chair5dof_rc_3dof).diff(t))
]
...
V_rear_road = (
    S.One / 2 * k_rt * (z_rear - u_rr_bump_on) ** 2 + S.One / 2 * k_r * (u_rear) ** 2
)  # Ep tylnich prętów ramy względem ich sprężystości
V_fr_road = (
    S.One / 2 * k_ft * (z_fr - u_rf_bump_on) ** 2 + S.One / 2 * k_f * (u_fr) ** 2
)  # Ep przednich prętów ramy względem ich sprężystości
T_chair5dof = T_body + T_rot + T_rear + T_fr + T_wheel
...
class Chair5DOF(dyn.LagrangesDynamicSystem):
    def __init__(
        self,
        Lagrangian=L_default_5dof,
        qs=qs_5dof,
        forcelist=FL_chair5dof,
        bodies=None,
        frame=N,
        hol_coneqs=None,
        nonhol_coneqs=None,
...
class RapidChair3DOF(dyn.LagrangesDynamicSystem):
    def __init__(
        self,
        Lagrangian=L_rc_3dof,
        qs=qs_rc_3dof,
        forcelist=FL_rapidchair3dof,
        bodies=None,
        frame=N,
        hol_coneqs=None,
        nonhol_coneqs=None,
...
class Chair5DOFwithRC3DOF(dyn.LagrangesDynamicSystem):
    def __init__(
        self,
        Lagrangian=L_chair5dof_rc3dof,
        qs=qs_chair5dof_rc_3dof,
        forcelist=FL_chair5dof_rc3dof,
        bodies=None,
        frame=N,
        hol_coneqs=None,
        nonhol_coneqs=None,
...
chair_3dof_rev = chair_5dof.subs(dof3_rev, method="direct").shranked(x, z_rear, z)(
    "Chair 3DOF model with revolution"
)
chair_3dof_norev = chair_5dof.subs(dof3_norev, method="direct").shranked(x, z, phi)(
    "Chair 3DOF model without revolution"
)
...
rapidchair_2dof = rapidchair_3dof.subs(rcdof2, method="direct").shranked(theta, phi_rc)(
    "RapidChair 2DOF model"
)
rapidchair_1dof = rapidchair_3dof.subs(rcdof1, method="direct").shranked([phi_rc])(
    "RapidChair 1DOF model"
)

chair5dof_rc3dof = Chair5DOFwithRC3DOF()

### system parameters
m_fr, m_rear, m_3, k_r, k_rt, k_f, k_ft, k_rot = symbols(
    "m_fr, m_r, M, k_r, k_rt, k_f, k_ft k_rot", positive=True
)
m, k, g, F_1, F_2, Omega, F, R, v0, u0, l_l, l_r = symbols(
    "m,k,g,F_1,F_2,Omega, F_0 R, v_0,u_0,l_l,l_r", positive=True
)
...
V_rear = (
    S.One / 2 * k_rt * (z_rear - amplitude * cos(2 * pi / (length / speed) * t)) ** 2
)  # Ep tylnich prętów ramy względem ich sprężystości
V_fr = (
    S.One
    / 2
    * k_ft
    * (z_fr - amplitude * cos(2 * pi / (length / speed) * (t - axw / speed))) ** 2
)  # Ep przednich prętów ramy względem ich sprężystości

# +++++++++++++++++++++
V_pion = (
    S.One / 2 * k_r * (z - z_rear) ** 2 + S.One / 2 * k_f * (z - z_fr) ** 2
)  ##### te
# +++++++++++++++++++++

##               k_rear                                    k_fr
V_obr = (
    S.One / 2 * k_r * (z + l_l * phi - z_rear) ** 2
    + S.One / 2 * k_f * (z - l_r * phi - z_fr) ** 2
    - S.One / 2 * m_3 * z_c3 * g * phi**2
)
...
Dlam_chair5dof = (
    (chair_5dof_lin.q.diff(t)).T
    * Matrix([Epotential]).jacobian(chair_5dof_lin.q).jacobian(chair_5dof_lin.q)
    * chair_5dof_lin.q.diff(t)
    * c_lam
)[0]
Dmu_chair5dof = (
    (chair_5dof_lin.q.diff(t)).T
    * Matrix([Ekinetic])
    .jacobian(chair_5dof_lin.q.diff(t))
    .jacobian(chair_5dof_lin.q.diff(t))
    * chair_5dof_lin.q.diff(t)
    * c_mu
)[0]

FL_SIMPLECHAIR = [(Pl, 0.5 * (1 * sign(cos(Omega * t) - pm) + 1) * 2 * F * N.x)] + [
    (points_list[nom], (-Dmu_chair5dof.diff(velm) - Dlam_chair5dof.diff(velm)) * N.x)
    for nom, velm in enumerate(Matrix(qs_5dof).diff(t))
]
    ...
    def __init__(
        ...
        **kwargs
        ...
        V_rear = (
            ...
            / 2
            * k_rt
            * (z_rear - amplitude * cos(2 * pi / (length / speed) * t)) ** 2
        )  # Ep tylnich prętów ramy względem ich sprężystości
        V_fr = (
            S.One
            / 2
            * k_ft
            * (z_fr - amplitude * cos(2 * pi / (length / speed) * (t - axw / speed)))
            ** 2
        )  # Ep przednich prętów ramy względem ich sprężystości
