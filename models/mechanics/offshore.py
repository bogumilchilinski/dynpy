...
class VesselWithHeaveAndRotation(ComposedSystem):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.M_matrix = Matrix(
            [[self.m_vessel, 0], [0, self.I_5]]
        )  # wszedzie teraz pobiera None # dopisz wszędzie self tut
        self.K_matrix = Matrix(
            [
                [
                    self.rho * self.g * self.A_wl,
                    -self.rho * self.g * self.A_wl * (self.CoF - self.CoB),
                ],  # trzeba ify popsać? #ify jużsą tylko na to H i Phi brakue chyba
                [
                    -self.rho * self.g * self.A_wl * (self.CoF - self.CoB),
        ...
        self.potential_energy = S.Half * sum(
            Matrix(self.qs).T * self.K_matrix * Matrix(self.qs)
        )
    ...
    def components(self):
        ...
        self._vessel_inertia = Element(self.kinetic_energy, qs=self.qs)
        self._buoyancy_stiffness = Element(-self.potential_energy, qs=self.qs)
...
class TDoFCompensatedPayload(ComposedSystem):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(HarmonicOscillator, self).__init__(
            (self.kinetic_energy - self.potential_energy).subs(positions_dict).doit(),
            qs=qs,
            ivar=ivar,
            **kwargs
        )
    ...
    def single_dof_transformation(self, p=None):
        ...
        nonlin_mathieu_model = sdof_payload.subs(
            {self.h: 0, self.h_c: 0, self.m_c: 0}
        ).subs(
            {self.h_eq: self.l_c, self.h_ceq: self.l_c, self.l_0: self.l_w - self.l_c}
        )
        ...
        mathieu_basis = mathieu_acc_form.subs(
            {self.l_c: 0, self.g: omega**2 * self.l_w}
        )

        mathieu_instability_eq = mathieu_basis.subs(
            S.Half * omega**2 * self.phi**2,
            (S.Half) ** 3 * omega**2 * self.phi**2 * (1 + eps * delta),
            simultenouos=True,
        )

        mathieu_transformed_eq = mathieu_instability_eq.subs(
            {
                self.l_w: 1 / eps,
    ...
    def symbols_description(self):
        ...
        self.sym_desc_dict = {
            ...
            **{
                ...
                self.y_e: r"lateral displacement at crane tip obtained from RAOs (a regular wave excitation),",
                self.z_e: r"vertical displacement at crane tip obtained from RAOs (a regular wave excitation),",
                self.m_c: r"mass of compensator,",
                self.k_c: r"stiffness of heave compensator,",
                self.l_c: r"length of the attached compensating element,",
                self.g: r"acceleration of gravity,",
    ...
    def numerical_data(self):
        numbers_dict = {
            self.h_eq: ((self.m_c + self.m_p) / self.k_w + (self.m_p) / self.k_c)
            * self.g,
...
class CompoundSystem(ComposedSystem):
    ...
    def components(self):
        ...
        self._material_point = MaterialPoint(self._p, self.qs[0], self.qs)(
            "Material Point"
        )
...
class SDOFSemiSubmersiblePlatform(ComposedSystem):
    ...
    def components(self):
        ...
        self._platform = MaterialPoint(self.m, pos1=self.z, qs=[self.z])(
            label="Material point - mass of the platform"
        )
        self._spring_1 = Spring(
            ...
            qs=[self.z],
        )(label="Spring - stiffness of the platform")
        self._spring_2 = Spring(
            self.p_p * self.rho,
            pos1=(self.k_1 * self.z) / (self.p_p * self.rho),
            qs=[self.z],
        )(label="Spring - buoyancy")
        self._damper = Damper(
            self.c_1,
            pos1=self.z,
            pos2=(self.k_1 * self.z) / (self.p_p * self.rho),
        ...
        self._water = Damper(
            self.c_2, pos1=(self.k_1 * self.z) / (self.p_p * self.rho), qs=[self.z]
        )(label="Damper - damping of the water")
        self._force = Force(-self.F_load, pos1=self.z, qs=[self.z])(
            label="Force - load on the platform"
        )
        self._MassOfPlatform = Force(-self.m * self.g, pos1=self.z, qs=[self.z])(
        ...
        self._excitation = Force(
            ...
            pos1=(self.k_1 * self.z) / (self.p_p * self.rho),
            qs=[self.z],
        )(label="Force - Force caused by the waves")
    ...
    def get_default_data(self):
        ...
        m0, Omega0, k0, p_p0, F0, lam0 = (
            Symbol("m_0", positive=True),
            Symbol("Omega0", positive=True),
            Symbol("k0", positive=True),
            Symbol("p_{p0}", positive=True),
            Symbol("F0", positive=True),
            Symbol("lambda0", positive=True),
        )
        c_2 = self.c_2
        g = self.g
        rho = self.rho
...
class DDOFSemiSubmersiblePlatform(ComposedSystem):
    ...
    def components(self):
        ...
        self._pontoon = MaterialPoint(self.m_p, pos1=self.z_p, qs=[self.z_p])(
            label="Material point - mass of the pontoon"
        )
        self._spring_1 = Spring(
            self.k_1, pos1=self.z, pos2=self.z_p, qs=[self.z, self.z_p]
        )(label="Spring - stiffness of the platform")
        self._spring_2 = Spring(
            self.p_p * self.rho, pos1=self.z_p, qs=[self.z_p, self.z]
        )(label="Spring - buoyancy")
        self._damper = Damper(
            self.c_1, pos1=self.z, pos2=self.z_p, qs=[self.z, self.z_p]
        )(label="Damper - damping of the platform")
        self._water = Damper(self.c_2, pos1=self.z_p, qs=[self.z_p, self.z])(
            label="Damper - damping of the water"
        )
        self._force = Force(-self.F_load, pos1=self.z, qs=[self.z, self.z_p])(
            label="Force - load on the platform"
        )
        self._MassOfPlatform = Force(
            -self.m * self.g, pos1=self.z, qs=[self.z, self.z_p]
        )(label="Force - weight of the platform in gravitational field")
        self._MassOfPontoon = Force(
            -self.m_p * self.g, pos1=self.z_p, qs=[self.z_p, self.z]
        )(label="Force - weight of the pontoon in gravitational field")
        self._excitation = Force(
            self.F * sin(self.Omega * self.ivar), pos1=self.z_p, qs=[self.z_p, self.z]
        )(label="Force - Force caused by the waves")
...
class ROVImpactDamped(ComposedSystem):
    ...
    def __init__(self, m=None, c=None, z=None, z_s=None, ivar=Symbol("t"), **kwargs):
...
class ROVImpactElasticDamped(ROVImpactDamped):
    ...
    def __init__(
        self, m=None, c=None, k=None, z=None, z_s=None, ivar=Symbol("t"), **kwargs
    ):
