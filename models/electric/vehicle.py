...
class ElectricMotorcycle2DoF(ComposedSystem):
    ...
    def components(self):
        ...
        self.inertia = Inductor(
            -self.J, self.x / self.r, qs=self.qs, ivar=self.ivar, frame=base_frame
        )
        self.viscous_friction = Resistor(
            -self.B, self.x / self.r, qs=self.qs, ivar=self.ivar
        )
        self.electromotive_force = ElectromotiveForce(
            -self.k_m * self.charge.diff(self.ivar) * self.i,
            self.x / self.r,
            qs=self.qs,
            ivar=self.ivar,
            frame=base_frame,
        ...
        self.rotor_torque = RotorTorque(
            self.k_e * self.x.diff(self.ivar) / self.r,
            self.charge,
            qs=self.qs,
            ivar=self.ivar,
            frame=base_frame,
        ...
        self.air_drag = Force(
            S.One / 2 * self.Ad * self.Cd * self.Af * (self.x.diff(self.ivar)) ** 2,
            self.x,
            qs=self.qs,
            ivar=self.ivar,
        )
        self.pwm = Force(
            self.Rpwm
            * Heaviside(-(self.ivar) + self.time_cut)
            * Heaviside(self.charge.diff(self.ivar) - self.cut1)
            * self.charge.diff(self.ivar),
            self.charge,
            qs=self.qs,
            ivar=self.ivar,
            frame=base_frame,
        ...
        self.pwm2 = Force(
            self.Rpwm2
            * Heaviside((self.ivar) - self.time_cut)
            * Heaviside(self.charge.diff(self.ivar) - self.cut2)
            * self.charge.diff(self.ivar),
            self.charge,
            qs=self.qs,
            ivar=self.ivar,
    ...
    def required_power(self):
        ...
        preq_eq = Eq(
            self.preq,
            (
                self.m * self.g * self.ft
                + (self.Cd * self.Ad * (self.v**2) / 2 * self.Af)
            )
            / self.eta_p
            * self.v,
        )
    ...
    def as_first_order_em(self):
        ...
        damp_mat = (
            self.damping_matrix()
            .subs(self.charge, self.current)
            .subs(DiracDelta(self.current.diff(self.ivar) - self.cut1), 0)
            .subs(
                Heaviside(self.current.diff(self.ivar) - self.cut1),
                Heaviside(self.current - self.cut1),
            )
            .subs(
                self.Ad * self.Af * self.Cd * self.x.diff(self.ivar),
                self.Ad * self.Af * self.Cd * self.velocity * S.Half,
            )
            .subs(DiracDelta(self.current.diff(self.ivar) - self.cut2), 0)
            .subs(
                Heaviside(self.current.diff(self.ivar) - self.cut2),
                Heaviside(self.current - self.cut2),
            )
        )
        sec_to_first_comp = inert_mat * Matrix(
            [[self.current.diff(self.ivar)], [self.velocity.diff(self.ivar)]]
        )
        ...
        return ODESystem(
            -eoms_mat,
            Matrix([[self.current], [self.velocity]]),
            ivar=self.ivar,
            ode_order=None,
        )
...
class ElectricMotorcycle2DoFMSM(ElectricMotorcycle2DoF):
    ...
    def components(self):
        ...
        self.cut = self.ic
        r_pwm_1 = self.Rpwm * (
            Heaviside(self.charge.diff(self.ivar) - self.cut) * self.eps + 1
        )
        ...
        self.pwm = Force(
            r_pwm_1 * self.charge.diff(self.ivar),
            self.charge,
            qs=self.qs,
            ivar=self.ivar,
            frame=base_frame,
    ...
    def as_first_order_em(self):
        ...
        damp_mat = (
            self.damping_matrix()
            .subs(self.charge, self.current)
            .subs(DiracDelta(self.current.diff(self.ivar) - self.cut), 0)
            .subs(
                Heaviside(self.current.diff(self.ivar) - self.cut),
                Heaviside(self.current - self.cut),
            )
            .subs(
                self.Ad * self.Af * self.Cd * self.x.diff(self.ivar),
                self.Ad * self.Af * self.Cd * self.velocity * S.Half,
...
class RegenerativeBrakingElectricMotorcycle2DoFMSM(ElectricMotorcycle2DoFMSM):
    ...
    def components(self):
        components = {}
        r_pwm_1 = self.Rpwm * (
            1 + self.eps * Heaviside(self.charge.diff(self.ivar) + 20)
        )
...
class adtest(ComposedSystem):
    ...
    def __init__(self, Cd=None, Ad=None, Af=None, m=None, ivar=Symbol("t"), **kwargs):
