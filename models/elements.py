...
class GeometryOfPoint:
    ...
    def __init__(
        self, *args, frame=base_frame, base_origin=base_origin, ivar=Symbol("t")
    ):
        ...
        if isinstance(args[0], Point):
            ...
            if len(args) == 2:
                ...
                P1.set_vel(
                    frame, frame.x * diff(args[0], ivar) + frame.y * diff(args[1], ivar)
                )
                self.Point = P1
                # print(self.Point)
                ...
                P2.set_pos(
                    base_origin,
                    frame.x * args[0] + frame.y * args[1] + frame.z * args[2],
                )
                P2.set_vel(
                    frame,
                    frame.x * diff(args[0], ivar)
                    + frame.y * diff(args[1], ivar)
                    + frame.z * diff(args[2], ivar),
                )
                self.Point = P2
                # print(self.Point)
...
class Element(LagrangesDynamicSystem):
    ...
    def _get_virtual_work(self):
        return Eq(
            Symbol("\delta W"),
            Symbol(latex(self.force)) * Symbol(f"\delta({latex(self._pos1)})"),
        )
...
class MaterialPoint(Element):
    ...
    def __init__(self, m, pos1, qs=None, frame=base_frame, ivar=Symbol("t"), **kwargs):
        if m is not None:
            self.m = m
        if pos1 is not None:
            self.pos1 = pos1
        ...
        if isinstance(pos1, Point):
            ...
            Lagrangian = (
                S.Half
                * m
                * ((base_origin.pos_from(pos1).diff(ivar, frame).magnitude()) ** 2)
            )
    ...
    def _get_virtual_work(self):
        return Eq(
            Symbol("\delta W"),
            Symbol(latex(self.m * self.pos1.diff(self.ivar)))
            * Symbol(f"\delta({latex(self.pos1)})"),
        )
    ...
    @classmethod
    def from_velocity(
        cls, m, velocity, qs=None, z=None, ivar=Symbol("t"), frame=base_frame, **kwargs
    ):
    ...
    @classmethod
    def from_default(
        cls, m=None, pos1=None, qs=None, ivar=Symbol("t"), frame=base_frame, **kwargs
    ):
        if m is None:
            m = cls.m
        if pos1 is None:
            pos1 = dynamicsymbols("x")
        if not qs:
            qs = [pos1]
        ivar = cls.ivar
        return cls(m=m, pos1=pos1, qs=qs, ivar=ivar, frame=frame)
    ...
    def _plot_2d(self, language="en", *args, **kwargs):
        ...
        GeometryScene.ax_2d.text(
            2.5, 5, f"${self.m}$", rotation="horizontal", multialignment="center"
        )
...
class Spring(Element):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Lagrangian = (
            -S.Half
            * self.stiffness
            * ((self.pos2.pos_from(self.pos1)).magnitude() - self.l_s) ** 2
        )
    ...
    def force(self):
        if self.pos2 == 0:
            ...
            force = (
                self.stiffness * (self.pos2.pos_from(self.pos1)).magnitude() - self.l_s
            )
        return force
    ...
    def from_default(
        ...
        **kwargs,
        ...
        if stiffness is not None:
            cls.stiffness = stiffness
        if pos1 is None:
            pos1 = dynamicsymbols("z")
        if not qs:
            qs = [pos1]
        ivar = cls.ivar
...
class GravitationalForce(Element):
    ...
    def get_numerical_data(self):
        self.default_data_dict = {self.g: [9.81]}
        return self.default_data_dict
...
class RigidBody2D(Element):
    ...
    def __init__(
        self, m, I, pos_lin=0, pos_rot=0, qs=None, frame=base_frame, ivar=Symbol("t")
    ):
        ...
        if isinstance(pos_lin, Point) and isinstance(pos_rot, Point):
            Lagrangian = S.Half * m * (
                (base_origin.pos_from(pos_lin).diff(ivar, frame).magnitude()) ** 2
            ) + S.Half * I * (
                (base_origin.pos_from(pos_rot).diff(ivar, frame).magnitude()) ** 2
            )

        else:
            Lagrangian = (
                S.Half * m * diff(pos_lin, ivar) ** 2
                + S.Half * I * diff(pos_rot, ivar) ** 2
            )
...
class Damper(Element):
    ...
    def __init__(self, c, pos1, pos2=0, qs=None, ivar=Symbol("t"), frame=base_frame):
        ...
        if isinstance(pos1, Point):
            ...
            D = (
                S.Half
                * c
                * ((pos1.vel(frame) - pos2.vel(frame)).magnitude() ** 2).doit()
            )  # pos1.vel(frame).magntidue() behaves as diff(pos1,ivar)
        else:
        ...
        forcelist = [
            (point_dmp, -diff(D, coord_vel) * frame.x)
            for coord_vel, point_dmp in points_dict.items()
        ]
        # print(forcelist)
    ...
    def _plot_2d(self, language="en", *args, **kwargs):
        ...
        res = GeometryScene.ax_2d.plot(
            np.array([0, 0, 1, -1]) + x_shift + 2,
            np.array([-1, 2, 2, 2]) + y_shift,
            label=class_name,
            color="k",
        ) + GeometryScene.ax_2d.plot(
            np.array([1.5, 1.5, 0, 0, 0, -1.5, -1.5]) + x_shift + 2,
            np.array([1, 3, 3, 5, 3, 3, 1]) + y_shift,
            label=class_name,
            color="k",
        )

        text = GeometryScene.ax_2d.text(
            np.array([1.5]), np.array([3]), f"{class_name}", multialignment="center"
        )
...
class EngineMount(Spring):
    ...
    def get_numerical_data(self):
        self.default_data_dict = {self.stiffness: [1e5 * no for no in range(1, 8)]}
        return self.default_data_dict
...
class DampedEngineMount(EngineMount):
    ...
    def components(self):
        ...
        self._elasticity = EngineMount(
            self.stiffness, pos1=self.pos1, pos2=self.pos2, l_s=self.l_s, qs=self.qs
        )(label="Elasticity", scheme_options={"at": (2, 4)})
        self._damping = Damper(
            self.damping_coeff, pos1=self.pos1, pos2=self.pos2, qs=self.qs
        )(label="Damping", scheme_options={"at": (-2, 4)})
...
class Force(Element):
    ...
    def __init__(self, force, pos1=None, qs=None, ivar=Symbol("t"), frame=base_frame):
...
class CombustionEngine(Force):
    ...
    def __init__(
        ...
        super().__init__(
            self.eta * self._characteristic, omega.integrate(ivar), qs=qs, ivar=ivar
        )
    ...
    @classmethod
    def from_data(
        cls, data, n, degree=5, eta=None, qs=None, ivar=Symbol("t"), frame=base_frame
    ):
        ...
        window = ((S.One / 2 + atan(char_slope * (omega - omega_min)) / np.pi)) - (
            (S.One / 2 + atan(char_slope * (omega - omega_max)) / np.pi)
        )
        # window = Heaviside(omega-omega_min) - Heaviside(omega-omega_max)
...
class IntegralElement(Force):
    ...
    def __init__(
        ...
        if isinstance(self.error, Expr) and self.reference == S.Zero:
            ...
            super().__init__(
                k_I * (self.reference - self.error).integrate(ivar),
                target,
                qs=qs,
                ivar=ivar,
            )
    ...
    def _plot_2d(self, language="en"):
        ...
        res = (
            GeometryScene.ax_2d.plot(x_coords, y_coords, label=class_name, color="k")
            + GeometryScene.ax_2d.plot([-1, 0], [1.5, 1.5], label=class_name, color="k")
            + GeometryScene.ax_2d.plot([6, 5], [1.5, 1.5], label=class_name, color="k")
        )

        text = GeometryScene.ax_2d.text(
            np.array([1]), np.array([1.5]), f"{class_name}", multialignment="center"
        )
    ...
    def _tikz_scheme(self, language="en"):
        ...
        with schm.create(TikZ()) as tkz:
            with tkz.create(
                TikZNode("Integral Element", options=["draw"], text=f"{class_name}")
            ) as node:
                pass
            with tkz.create(
                TikZNode("Integral Element", options=["draw"], text=f"{class_name}")
...
class DerivativeElement(Force):
    ...
    def __init__(
        ...
        if isinstance(self.error, Expr) and self.reference == S.Zero:
            ...
            super().__init__(
                k_D * diff((self.reference - self.error), ivar),
                target,
                qs=qs,
                ivar=ivar,
            )
    ...
    def _plot_2d(self, language="en"):
        ...
        text = GeometryScene.ax_2d.text(
            np.array([1.2]), np.array([1.5]), f"{class_name}", multialignment="center"
        )
    ...
    def _tikz_scheme(self, language="en"):
        ...
        with schm.create(TikZ()) as tkz:
            with tkz.create(
                TikZNode("Derivative Element", options=["draw"], text=f"{class_name}")
            ) as node:
                pass
            with tkz.create(
                TikZNode("Derivative Element", options=["draw"], text=f"{class_name}")
...
class ProportionalElement(Force):
    ...
    def __init__(
        ...
        if isinstance(self.error, Expr) and self.reference == S.Zero:
            ...
            super().__init__(
                k_P * (self.reference - self.error), target, qs=qs, ivar=ivar
            )
    ...
    def _tikz_scheme(self, language="en"):
        ...
        with schm.create(TikZ()) as tkz:
            with tkz.create(
                TikZNode("Proportional Element", options=["draw"], text=f"{class_name}")
            ) as node:
                pass
            with tkz.create(
                TikZNode("Proportional Element", options=["draw"], text=f"{class_name}")
    ...
    def __init__(self, kp, ki, kd, pos1, qs=None, ivar=Symbol("t"), frame=base_frame):
        ...
        if isinstance(pos1, Point):
            D = (
                ...
                * c
                * ((pos1.vel(frame) - pos2.vel(frame)).magnitude() ** 2).doit()
            )  # pos1*ki + pos1.vel(frame)*kp + d2pos1*kd
        else:
            D = S.Half * c * (diff(pos1 - pos2, ivar)) ** 2
        ...
        forcelist = [
            (point_dmp, -diff(G, coord_vel) * frame.x)
            for coord_vel, point_dmp in points_dict.items()
        ]
...
class ConstrainsReaction(Element):
    ...
    def __init__(
        self, constrain=None, pos1=None, qs=None, ivar=Symbol("t"), frame=base_frame
    ):
