...
class LinearSecondOrderHeavisideComp(ODESystem):
    ...
    def from_reference_data(cls):
        ...
        omg = Symbol("Omega", positive=True)
        ode_eq = (
            z.diff(t, 2) + omg0**2 * z - fd * Heaviside(sin(omg * t)) - fs
        )  # +2*h*z.diff(t)
...
class SpringMassDamperExcitedMSM(MultiTimeScaleSolution):
    ...
    def from_reference_data(cls):
        ...
        MSM_eq = Eq(
            x.diff(t, 2) + eps * diff(x) + omega_0**2 * x, eps * mu * sin(Omega * t)
        )
...
class SpringMassDamperExcitedSP(ODESystem):
    ...
    def from_reference_data(cls):
        ...
        MSM = cls(MSM_eq.lhs - MSM_eq.rhs, dvars=x)
        #         exc_dict = {omega_0:Omega**2+eps*delta}
        #         MSM_sol_sub = MSM.solution.subs(exc_dict)
...
class SpringMassExcitedMSM(ODESystem):
...
class HarmonicOscillator(ODESystem):
    ...
    def from_reference_data(cls):
        ...
        omg = Symbol("omega")
        param_dict = {
            system.F: 1 * cos(omg * system.ivar),
            system.k: omg,
            system.g: 0,
            system.m: 1,
        }
        na_df = dyn_sys.subs(param_dict).eoms.numerical_analysis(
            parameter=omg, param_span=k_list, t_span=t_span
        )
        na_df_sol = na_df.compute_solution(t_span=t_span, ic_list=[0.1, 0.1]).iloc[
            :, sym_list
        ]
        plot = na_df_sol.plot()
        plt.show()
...
class ParametricOscillator(ODESystem):
    ...
    def from_reference_data(cls):
        ...
        omg = Symbol("omega")
        param_dict_k = {
            system.k: 1 + 0.01 * cos(omg * system.ivar),
            system.F: 1 * cos(omg * system.ivar),
            system.g: 0,
            system.m: 1,
        }
        na_df_k = dyn_sys.subs(param_dict_k).eoms.numerical_analysis(
            parameter=omg, param_span=k_list, t_span=t_span
        )
        na_df_sol_k = na_df_k.compute_solution(t_span=t_span, ic_list=[0.1, 0.1]).iloc[
            :, sym_list
        ]
